...
public class DefaultFlashBuyService implements FlashBuyService
{
	...
	{
		validateParameterNotNull(promotionSourceRule, "Promotion source rule must not be null");

		promotionSourceRule.getEngineRules().stream().forEach(engineRule -> {
			final String moduleName = RuleMappings.moduleName((DroolsRuleModel) engineRule);
	...
	{
		validateParameterNotNull(sourceRule, "Promotion source rule must not be null");
	...
	{
		validateParameterNotNull(rule, "Promotion source rule must not be null");
