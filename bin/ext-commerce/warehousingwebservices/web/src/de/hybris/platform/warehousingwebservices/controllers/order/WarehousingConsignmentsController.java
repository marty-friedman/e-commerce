...
public class WarehousingConsignmentsController extends WarehousingBaseController
{
	...
	public ConsignmentWsDTO getConsignmentForCode(@ApiParam(value = "The consignment code") @PathVariable final String code,
			@ApiParam(value = "Fields mapping level", required = false) @RequestParam(required = false, defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
	...
	public WarehouseSearchPageWsDto getSourcingLocationsForConsignmentCode(
			@ApiParam(value = "The consignment code") @PathVariable final String code,
			@ApiParam(value = "Fields mapping level", required = false) @RequestParam(required = false, defaultValue = DEFAULT_FIELD_SET) final String fields,
			@ApiParam(value = "Current page", required = false) @RequestParam(required = false, defaultValue = DEFAULT_CURRENT_PAGE) final int currentPage,
			@ApiParam(value = "Page size", required = false) @RequestParam(required = false, defaultValue = DEFAULT_PAGE_SIZE) final int pageSize,
			@ApiParam(value = "Sort parameter", required = false) @RequestParam(required = false, defaultValue = DEFAULT_SORT) final String sort)
	 ...
	 * @param code
	 * 		{@link ConsignmentModel#CODE} for the requested consignment
	 * @return the generated html Pick Slip template
	 */
	@RequestMapping(value = "/{code}/pick", method = RequestMethod.POST, produces = { MediaType.ALL_VALUE })
	@ResponseStatus(HttpStatus.OK)
	@ResponseBody
	@ApiOperation(value = "Request to pick a given Consignment and generate its Pick Slip", response = String.class)
	...
	{
		return warehousingConsignmentFacade.pickConsignment(code);
	}
	 ...
	 */
	@RequestMapping(value = "/consolidated-pick", method = RequestMethod.POST, produces = {
			MediaType.ALL_VALUE }, consumes = { MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE })
	@ResponseStatus(HttpStatus.OK)
	@ResponseBody
	...
	/**
	 * Request to pack a {@link ConsignmentModel} and generate its Pack Label
	 *
	 * @param code
	 * 		{@link ConsignmentModel#CODE} for the request consignment
	 * @return the generated html Pack Label template
	 */
	@RequestMapping(value = "/{code}/pack", method = RequestMethod.POST, produces = {  MediaType.ALL_VALUE })
	@ResponseStatus(HttpStatus.OK)
	@ResponseBody
	@ApiOperation(value = "Request to pack a given Consignment and generate its Pack Label", response = String.class)
	public String packConsignment(
			@ApiParam(value = "Code corresponding to the desired consignment", required = true) @PathVariable @NotNull final String code)
	{
		return warehousingConsignmentFacade.packConsignment(code);
	}
