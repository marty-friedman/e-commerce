 ...
 */
package de.hybris.platform.assistedservicefacades.jalo;
 ...
 */
public class AssistedservicefacadesManager extends GeneratedAssistedservicefacadesManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(AssistedservicefacadesManager.class.getName());
	 ...
	 */
	public static AssistedservicefacadesManager getInstance()
	{
		return (AssistedservicefacadesManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				AssistedservicefacadesConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public AssistedservicefacadesManager() // NOPMD 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of AssistedservicefacadesManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of AssistedservicefacadesManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of AssistedservicefacadesManager called, current tenant: " + getTenant().getTenantID());
		}
	}
