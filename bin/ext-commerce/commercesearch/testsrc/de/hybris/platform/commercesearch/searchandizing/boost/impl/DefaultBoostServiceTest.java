...
public class DefaultBoostServiceTest
{
	...
	public void setUp()
	{
		...
		defaultBoostService.setSolrFacetSearchConfigDao(solrFacetSearchConfigDao);
	...
	{
		//given
		given(solrFacetSearchConfigDao.findFacetSearchConfigByName(anyString())).willReturn(facetSearchConfigModel);
		...
		given(guidedNavConfigDao.getSolrBoostSearchProfiles(anyCollection(), any(SolrIndexedTypeModel.class))).willReturn(
				categoryProfiles);
		given(categoryGuidedNavConfigMatchStrategy.findExactMatch(categoryProfiles, leafCategory))
				.willReturn(categorySearchProfile);

		//when
		final Collection<SolrBoostRuleModel> result = defaultBoostService.getBoostRulesForCategoryAndIndexedType(new IndexedType(),
				leafCategory, new FacetSearchConfig());
	...
	public void shouldReturnRootCategoryProfileBoostRules()
	{
		...
		given(categoryGuidedNavConfigMatchStrategy.findExactMatch(categoryProfiles, rootCategory))
				.willReturn(categorySearchProfile);
		given(categoryGuidedNavConfigMatchStrategy.findNearestMatch(anyCollection(), eq(categoryPath), eq(leafCategory)))
				.willReturn(categorySearchProfile);
	...
	public void shouldReturnGlobalProfileBoostRules()
	{
		...
		given(categoryGuidedNavConfigMatchStrategy.findNearestMatch(anyCollection(), eq(categoryPath), eq(leafCategory)))
				.willReturn(null);
		given(
				globalSearchProfileService.getGlobalSolrSearchProfileForIndexedType(any(IndexedType.class),
	...
	public void shouldReturnEmptyListProfileBoostRules()
	{
		...
		given(categoryGuidedNavConfigMatchStrategy.findNearestMatch(anyCollection(), eq(categoryPath), eq(leafCategory)))
				.willReturn(null);
		given(guidedNavConfigDao.findGlobalSolrSearchProfiles(any(SolrIndexedTypeModel.class))).willReturn(Collections.emptyList());
	...
	public void shouldReturnNullIfProfileDoesNotExist()
	{
		//given
		given(solrFacetSearchConfigDao.findFacetSearchConfigByName(anyString())).willReturn(facetSearchConfigModel);
		given(guidedNavConfigDao.findCategorySolrSearchProfiles(any(SolrIndexedTypeModel.class), eq(leafCategory.getCode())))
				.willReturn(Collections.emptyList());
		given(
				categorySearchProfileService.getCategorySolrSearchProfile(eq(leafCategory.getCode()), any(SolrIndexedTypeModel.class)))
				.willReturn(null);

		//when
		final AbstractSolrSearchProfileModel result = defaultBoostService.getProfileForCategoryAndIndexedType(new IndexedType(),
				leafCategory, new FacetSearchConfig());
