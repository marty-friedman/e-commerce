...
public class DefaultBoostServiceIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void setUp() throws Exception
	{
		...
		importService.importData(new StreamBasedImpExResource(
				DefaultBoostServiceIntegrationTest.class.getResourceAsStream("/test/impex/common/essential-data.impex"),
				CSVConstants.HYBRIS_ENCODING, Character.valueOf(';')));

		importService.importData(
				new StreamBasedImpExResource(
						DefaultBoostServiceIntegrationTest.class.getResourceAsStream("/test/impex/catalog.impex"),
						CSVConstants.HYBRIS_ENCODING, Character.valueOf(';')));
		importService
				.importData(new StreamBasedImpExResource(
						DefaultBoostServiceIntegrationTest.class.getResourceAsStream("/test/impex/solr.impex"),
						CSVConstants.HYBRIS_ENCODING, Character.valueOf(';')));
	...
	@Test
	public void returnsNullAsProfileForCategory()
	{
		// given
		// no search profile created for the main category

		// when
		final AbstractSolrSearchProfileModel solrSearchProfile = boostService.getProfileForCategoryAndIndexedType(indexedType,
				mainCategory, facetSearchConfig);
		// then
		assertNull(solrSearchProfile);
	}

	@Test
	public void returnsCorrectProfileForCategory()
	{
		// given
	...
	@Test
	public void returnsGlobalProfileIfNoCategoryProvided()
	{
		// given
		...
		// when
		final AbstractSolrSearchProfileModel solrSearchProfile = boostService.getProfileForCategoryAndIndexedType(indexedType, null,
				facetSearchConfig);
