		...
		implements VendorProductSearchFacade
{
	...
	public VendorData getVendorCategories(final String vendorCode)
	{
		...
		if (optional.isPresent())
		{
			...
			SolrSearchQueryTermData solrSearchQueryTermData = new SolrSearchQueryTermData();
			...
			{
				List<ProductModel> productlist = marketplaceProductService.getAllProductByVendor(vendorCode);
	...
	private void convertVendorData(VendorData vendorData, List<ProductModel> productlist)
	{
		Iterator<ProductModel> iter = productlist.iterator();
		while (iter.hasNext())
		{
			ProductModel product = iter.next();
			List<CategoryData> categoryList = Converters.convertAll(product.getSupercategories(), categoryConverter);
