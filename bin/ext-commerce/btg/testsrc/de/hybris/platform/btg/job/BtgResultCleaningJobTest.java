...
public class BtgResultCleaningJobTest extends BTGIntegrationTest
{
	...
	/**
	 * Method checking if session result are stored in database If they are stored only in cache tests will not work
	 */
	private boolean sessionResultNotInDatabase()
	{
		return Config.getBoolean(BtgConstants.SESSION_SCOPE_RESULT_NOT_IN_DATABASE_PROPERTY, false);
	}

	@Test
	public void testNoResultToRemove()
	{
		final PerformResult result = btgResultCleaningJob.perform(modelService.create(BTGResultCleaningCronJobModel.class));
	...
	{
		//given
		createResults(BTGResultScope.PERMANENT, customerA);
	...
	@Test
	public void testCleanSessionResult() throws Exception
	{
		if (sessionResultNotInDatabase())
		{
			return;
		}
	...
	public void testCleanOnlyOldResults() throws Exception
	{
		...
		//when
		final PerformResult result = btgResultCleaningJob.perform(createCronJobModel(200, BTGResultScope.PERMANENT));

		//then
		Assert.assertEquals(CronJobResult.SUCCESS, result.getResult());
		Assert.assertEquals(CronJobStatus.FINISHED, result.getStatus());
		assertSegmentResultExists(3, BTGResultScope.PERMANENT);
	}
