...
public class MarketplaceVendorDirectoryScanner extends DefaultDirectoryScanner
{
	...
	public void setFileNamePatten(String fileNamePatten)
	{
	...
	@Override
	protected File[] listEligibleFiles(File root) throws IllegalArgumentException
	{
		Stream<File> files = Arrays.stream(root.listFiles()).filter(File::isDirectory).flatMap(d -> Arrays.stream(d.listFiles()))
				.filter(File::isFile)
