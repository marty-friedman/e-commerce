...
public class DefaultVendorCMSStrategy implements VendorCMSStrategy
{
	...
	private static final String CONTENT_SLOT_FOR_PAGE_ID_PATTERN = "BodyContentSlot-{0}-{1}";
	...
	public AbstractPageModel prepareLandingPageForVendor(final VendorModel vendor)
	{
		...
		final String pageName = vendorCode.toUpperCase() + SPACE + getStringConfiguration(LANDING_PAGE_NAME_SUFFIX);
			...
			final AbstractPageModel page, final AbstractCMSComponentModel... component)
	{
		...
		final String contentSlotForPageId = MessageFormat.format(CONTENT_SLOT_FOR_PAGE_ID_PATTERN, position, page.getUid());
