...
public class Orders extends AbstractItems<OrderModel>
{
	...
	public static final Double CAMERA_PRICE = 23.00;
	public static final Double MEMORY_CARD_PRICE = 17.00;
	public static final Double LENS_PRICE = 13.00;
	...
	protected OrderEntryModel Camera(final Long quantity)
	{
		...
		return OrderEntryModelBuilder.fromModel(Default(quantity)).withBasePrice(CAMERA_PRICE)
				.withTotalPrice(quantity * CAMERA_PRICE).withProduct(getProducts().Camera()).withTaxes(taxes)
				.withCalculated(Boolean.TRUE).build();
	}
	...
	protected OrderEntryModel MemoryCard(final Long quantity)
	{
		ArrayList<TaxValue> taxes = new ArrayList<TaxValue>();
		taxes.add(new TaxValue(CODE_TAX, Tax_Value, Boolean.TRUE, getCurrencies().AmericanDollar().getIsocode()));
		return OrderEntryModelBuilder.fromModel(Default(quantity)).withBasePrice(MEMORY_CARD_PRICE)
				.withTotalPrice(quantity * MEMORY_CARD_PRICE).withProduct(getProducts().MemoryCard()).withTaxes(taxes)
				.withCalculated(Boolean.TRUE).build();
	}
	...
	protected OrderEntryModel Lens(final Long quantity)
	{
		ArrayList<TaxValue> taxes = new ArrayList<TaxValue>();
		taxes.add(new TaxValue(CODE_TAX, Tax_Value, Boolean.TRUE, getCurrencies().AmericanDollar().getIsocode()));
		return OrderEntryModelBuilder.fromModel(Default(quantity)).withBasePrice(LENS_PRICE).withTotalPrice(quantity * LENS_PRICE)
