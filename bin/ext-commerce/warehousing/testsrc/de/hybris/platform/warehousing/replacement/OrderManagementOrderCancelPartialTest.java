...
public class OrderManagementOrderCancelPartialTest extends OrderCancelPartialTest
{
	...
	public void testSomeCancelableOrderEntriesWithConsignments()
	{
		...
		Assert.assertTrue(cancelableEntries.containsKey(getOrder().getEntries().get(0)));
		Assert.assertFalse(cancelableEntries.containsKey(getOrder().getEntries().get(1)));
		Assert.assertTrue(cancelableEntries.containsKey(getOrder().getEntries().get(2)));

		Assert.assertEquals("All quantity of entry1 should be cancelable", Long.valueOf(1), cancelableEntries.get(getOrder().getEntries().get(0)));
