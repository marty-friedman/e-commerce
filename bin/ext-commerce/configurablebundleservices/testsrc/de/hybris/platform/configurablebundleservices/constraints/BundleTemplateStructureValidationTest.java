...
public class BundleTemplateStructureValidationTest extends AbstractBundleValidationTest
{
    ...
    @Test
    @Ignore("ConstraintValidatorContext#getDefaultConstraintMessageTemplate() returns annotation value instead of DB one")
    public void shouldTakeDefaultMessageFromImpex() throws ImpExException
    {
        importString(
                "UPDATE TypeConstraint;id[unique=true];message[lang='en']\n"
                + ";BundleTemplateProductsAndChildrenConstraint1;Test\n"
        ...
        thrown.expect(AssertionError.class);
        thrown.expectMessage(endsWith("Test\n"));
        importString(
    ...
    @Test
    @Ignore("ConstraintValidatorContext#getDefaultConstraintMessageTemplate() returns annotation value instead of DB one")
    public void shouldTakeCustomMessageFromImpex() throws ImpExException
    {
        importString(
                "UPDATE TypeConstraint;id[unique=true];message[lang='en']\n"
                + ";BundleTemplateRequiringIntegrityConstraint1;Test\n"
                + "$de.hybris.platform.core.Registry.getApplicationContext().getBean(de.hybris.platform.validation.services.ValidationService.class).reloadValidationEngine()\n"
        );
        thrown.expect(AssertionError.class);
        thrown.expectMessage(endsWith("Test\n"));
