...
public class DefaultRuleEngineCalculationServiceCreationMethodsTest
{
	...
	public void testCreateOrderDiscountRelativeOrderLowerLimitInvalid()
	{
		...
		final Percentage discountAmount = (Percentage) createOrderDiscount.getAmount();

		assertThat(discountAmount.getRate(), equalTo(BigDecimal.ZERO));
	}
	...
	public void testCreateOrderDiscountRelativeOrderLowerLimitValid()
	{
		...
		assertThat(discountAmount.getRate(), equalTo(amount));
	}
	...
	public void testCreateDiscountRAO()
	{
		...
		assertCreateDiscountRAO(new LineItemDiscount(new Percentage(BigDecimal.valueOf(1000, currency.getDigits()))));
		assertCreateDiscountRAO(new OrderDiscount(new Money(BigDecimal.valueOf(1000, currency.getDigits()), currency)));
		assertCreateDiscountRAO(new OrderDiscount(new Percentage(BigDecimal.valueOf(1000, currency.getDigits()))));
	...
	protected void assertCreateDiscountRAO(final AbstractDiscount discount)
	{
		...
		{
			final Percentage amount = (Percentage) discount.getAmount();
			assertThat(createDiscountRAO.getValue(), is(amount.getRate()));
		}
		else
		{
