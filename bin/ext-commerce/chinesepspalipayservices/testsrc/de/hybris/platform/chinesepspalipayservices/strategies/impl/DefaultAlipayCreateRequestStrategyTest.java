...
public class DefaultAlipayCreateRequestStrategyTest
{
	...
	public void setUp() throws Exception
	{
		...
		DefaultAlipayCreateRequestStrategy testCreateRequestStrategy = new DefaultAlipayCreateRequestStrategy();
	...
	public void testCreateDirectPayUrl() throws Exception
	{
		...
		PowerMockito.when(AlipayUtil.generateUrl(any(), any())).thenReturn("requestString");

		String requestUrl = defaultAlipayCreateRequestStrategy.createDirectPayUrl(new AlipayDirectPayRequestData());
	...
	{
		doReturn(new HashMap<>()).when(defaultAlipayCreateRequestStrategy).describeRequest(any());
		PowerMockito.when(AlipayUtil.postRequest(any(), any())).thenReturn("");

		AlipayRawPaymentStatus result = defaultAlipayCreateRequestStrategy
	...
	{
		doReturn(new HashMap<>()).when(defaultAlipayCreateRequestStrategy).describeRequest(any());
		PowerMockito.when(AlipayUtil.postRequest(any(), any()))
				.thenReturn("<?xml version=\"1.0\" encoding=\"utf-8\"?><alipay><is_success>T</is_success></alipay>");
		AlipayRawPaymentStatus resultData = new AlipayRawPaymentStatus();
	...
	public void testSubmitCancelPaymentRequest() throws ReflectiveOperationException
	{
		...
		PowerMockito.when(AlipayUtil.postRequest(any(), any()))
				.thenReturn("<?xml version=\"1.0\" encoding=\"utf-8\"?><alipay><is_success>T</is_success></alipay>");
		AlipayRawCancelPaymentResult alipayRawCancelPaymentResult = new AlipayRawCancelPaymentResult();
		...
		AlipayRawCancelPaymentResult result = defaultAlipayCreateRequestStrategy
	...
	public void testCreateRefundUrl() throws Exception
	{
		...
		String refundRequest = defaultAlipayCreateRequestStrategy.createRefundUrl(new AlipayRefundRequestData());
	...
	{
		AlipayPaymentStatusRequestData requestData = new AlipayPaymentStatusRequestData();
		...
		Map<String, String> map = defaultAlipayCreateRequestStrategy.describeRequest(requestData);
	...
	public void testParserXML()
	{
		...
		AlipayRawPaymentStatus alipayRawPaymentStatus = (AlipayRawPaymentStatus) defaultAlipayCreateRequestStrategy
	...
	{
		String camelCase = "outTradeNo";

		String snakeCase = defaultAlipayCreateRequestStrategy.covert2SnakeCase(camelCase);
