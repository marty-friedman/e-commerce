...
public class OrdermanagementReturnPopulator implements Populator<ReturnRequestModel, ReturnRequestData>
{
			...
			if (source.getOrder() != null)
			{
				...
				if (source.getOrder().getDeliveryCost() != null)
				{
					target.setDeliveryCost(getPriceDataFactory()
							.create(PriceDataType.BUY, new BigDecimal(source.getOrder().getDeliveryCost()), currency));

					addDeliveryCost(source, target, total);
				}
	 ...
	 * 		the current total
	 */
	protected void addDeliveryCost(final ReturnRequestModel source, final ReturnRequestData target, final BigDecimal total)
	{
		if (source.getRefundDeliveryCost())
		{
			total.add(target.getDeliveryCost().getValue());
		}
	}
