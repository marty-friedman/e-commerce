...
public class B2BUnitServiceTest extends ServicelayerTransactionalTest
{
	...
	@Resource
	private B2BUnitService<B2BUnitModel, B2BCustomerModel> b2bUnitServiceProxy;
	@Resource
	...
	public void testRestrictionsOfUsersAndUnits() throws Exception
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				Assert.assertNull(b2bUnitServiceProxy.getUnitForUid("GC"));
	...
	public void testGetBranch() throws Exception
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				B2BUnitModel unit = b2bUnitServiceProxy.getUnitForUid("IC");
				Assert.assertNotNull(unit);
				final Set<B2BUnitModel> decendents = b2bUnitServiceProxy.getBranch(unit);
				Assert.assertNotNull(decendents);
				Assert.assertEquals(11, decendents.size());

				// test getAllUnitsOfOrganization
				final Set<B2BUnitModel> allUnits = b2bUnitServiceProxy.getBranch(unit);
				Assert.assertNotNull(allUnits);
				Assert.assertEquals(11, allUnits.size());


				unit = b2bUnitServiceProxy.getUnitForUid("IC Sales");
				Assert.assertNotNull(unit);
				Assert.assertEquals(b2bUnitServiceProxy.getParent(unit).getUid(), "IC");
				Assert.assertEquals(b2bUnitServiceProxy.getRootUnit(unit).getUid(), "IC");
				Assert.assertEquals(9, b2bUnitServiceProxy.getBranch(unit).size());
			}
		}, userService.getAdminUser());
	...
	public void testGetApprovalProcessCode() throws Exception
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				final B2BUnitModel unit = b2bUnitServiceProxy.getUnitForUid("IC");
				Assert.assertNotNull(unit);
				// test
				approvalCode = b2bUnitServiceProxy.getApprovalProcessCodeForUnit(unit);
				...
				final B2BUnitModel unitSales = b2bUnitServiceProxy.getUnitForUid("IC Sales");
				Assert.assertNotNull(unitSales);
				approvalCode = b2bUnitServiceProxy.getApprovalProcessCodeForUnit(unitSales);
				Assert.assertNotNull(approvalCode);
	...
	public void shouldUpdateBranchInSession() throws Exception
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				b2bUnitServiceProxy.updateBranchInSession(session, user);
	...
	public void shouldGetAllBranchAndParentUnitOfChildUnit()
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				login(userId);
				final Set<B2BUnitModel> allUnitsOfOrganization = b2bUnitServiceProxy
						.getAllUnitsOfOrganization(b2bCustomerService.getCurrentB2BCustomer());
	...
	public void shouldDisableBranchofParentUnit()
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				final B2BUnitModel unitUK = b2bUnitServiceProxy.getUnitForUid("GC Sales UK");

				b2bUnitServiceProxy.disableBranch(unitUK);

				LOG.debug("Test all units are disabled for : " + unitUK.getUid());

				final Set<B2BUnitModel> allUnitsOfOrganization = b2bUnitServiceProxy.getBranch(unitUK);
	...
	public void shouldGetParentUnitForCustomerWithMultipleUnitsAssigned()
	{
		sessionService.executeInLocalView(new SessionExecutionBody()
		{
			...
			public void executeWithoutResult()
			{
				...
				login(user);
				final B2BUnitModel parent = b2bUnitServiceProxy.getParent(user);
				Assert.assertNotNull(parent);
				// try to add the member which the user is already a member of
				b2bUnitServiceProxy.addMember(parent, user);
				...
				final B2BUnitModel unitUK = b2bUnitServiceProxy.getUnitForUid("GC Sales UK");
				b2bUnitServiceProxy.addMember(unitUK, user);
				modelService.save(user);
				Assert.assertEquals(2,
						CollectionUtils.select(user.getGroups(), PredicateUtils.instanceofPredicate(B2BUnitModel.class)).size());

				b2bUnitServiceProxy.updateParentB2BUnit(unitUK, user);
				modelService.save(user);

				Assert.assertEquals(unitUK, b2bUnitServiceProxy.getParent(user));
	...
	public void changeInActiveStateTriggersWarningMessage()
	{
		...
		final B2BUnitModel unit = b2bUnitServiceProxy.getUnitForUid("GC Sales UK");
	...
	public void login(final UserModel user)
	{
		...
		b2bUnitServiceProxy.updateBranchInSession(sessionService.getCurrentSession(), user);
	}
}
