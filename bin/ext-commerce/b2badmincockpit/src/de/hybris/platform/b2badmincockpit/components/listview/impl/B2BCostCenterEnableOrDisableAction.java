...
public class B2BCostCenterEnableOrDisableAction extends AbstractEnableDisableAction
{

	private static final Logger LOG = Logger.getLogger(B2BCostCenterEnableOrDisableAction.class);
	...
	public Menupopup getPopup(final Context context)
	{
		...
		final PropertyDescriptor pd = getTypeService().getPropertyDescriptor("B2BCostCenter.active");
				...
				public void onEvent(final Event event) throws Exception
				{
					...
					catch (final Exception e)
					{
						...
						Messagebox.show("Could not change active status of item (Reason: " + e.getMessage() + ").",
								Labels.getLabel("b2bitem.costcenter.enabledisable.exception"), Messagebox.OK, Messagebox.EXCLAMATION);
					}
					sendUpdateItemsEvent(context);
				}
	...
	{
		LOG.debug("Changing the status of B2BCostCenter...");

		final PropertyDescriptor pd = getTypeService().getPropertyDescriptor("B2BCostCenter.active");

		try
		{
			getValueService().getValue(selectedItem, pd);
		}
		catch (final Exception e)
		{
			...
			final B2BCostCenterModel b2bCostCenterModel = (B2BCostCenterModel) selectedItem.getObject();
			b2bCostCenterModel.setActive(Boolean.valueOf(activeStatus));

			getModelHelper().saveModel(b2bCostCenterModel, true, true);
			...
			{
				LOG.error("Could not change active status of item (Reason: " + e1.getMessage() + ").", e1);
			...
			valueContainer.addValue(pd, pd.isLocalized() ? UISessionUtils.getCurrentSession().getLanguageIso() : null, null);
			...
			if (valueContainer.isModified())
			{
				...
				UISessionUtils.getCurrentSession()
						.sendGlobalEvent(new ItemChangedEvent(this, selectedItem, Collections.singleton(pd)));
			}
		}
	}
}
