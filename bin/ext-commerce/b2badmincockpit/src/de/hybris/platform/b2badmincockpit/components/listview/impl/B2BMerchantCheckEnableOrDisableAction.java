...
public class B2BMerchantCheckEnableOrDisableAction extends AbstractEnableDisableAction
{
	...
	private static final Logger LOG = Logger.getLogger(B2BMerchantCheckEnableOrDisableAction.class);
	...
	public Menupopup getPopup(final Context context)
	{
		...
		final PropertyDescriptor pd = getTypeService().getPropertyDescriptor("B2BMerchantCheck.active");
				...
				public void onEvent(final Event event) throws Exception
				{
					...
					catch (final Exception e)
					{
						...
						Messagebox.show("Could not change active status of item (Reason: " + e.getMessage() + ").",
								Labels.getLabel("b2bitem.merchant.enabledisable.exception"), Messagebox.OK, Messagebox.EXCLAMATION);
					}
					sendUpdateItemsEvent(context);
				}
	...
	protected void doChangeStatus(final TypedObject selectedItem, final boolean activeStatus) throws Exception
	{
		...
		final B2BMerchantCheckModel b2bMerchantCheckModel = (B2BMerchantCheckModel) selectedItem.getObject();
		b2bMerchantCheckModel.setActive(Boolean.valueOf(activeStatus));

		getModelHelper().saveModel(b2bMerchantCheckModel, true, true);
