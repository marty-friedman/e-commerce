...
public class B2BPermissionEnableOrDisableAction extends AbstractEnableDisableAction
{

	private static final Logger LOG = Logger.getLogger(B2BPermissionEnableOrDisableAction.class);
	...
	public Menupopup getPopup(final Context context)
	{
		...
		final PropertyDescriptor pd = getTypeService().getPropertyDescriptor("B2BPermission.active");
				...
				public void onEvent(final Event event) throws Exception
				{
					...
					catch (final Exception e)
					{
						...
						Messagebox.show("Could not change active status of item (Reason: " + e.getMessage() + ").",
								Labels.getLabel("b2bitem.permission.enabledisable.exception"), Messagebox.OK, Messagebox.EXCLAMATION);
					}
					sendUpdateItemsEvent(context);
				}
	...
	{
		LOG.debug("Changing the status of B2BMerchant...");
		...
		final B2BPermissionModel b2bPermissionModel = (B2BPermissionModel) selectedItem.getObject();
		b2bPermissionModel.setActive(Boolean.valueOf(activeStatus));

		getModelHelper().saveModel(b2bPermissionModel, true, true);
