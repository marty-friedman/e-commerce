...
public class DefaultAsCacheStrategyTest
{
	...
	public void setUp() throws Exception
	{
		...
		when(Boolean.valueOf(configuration.getBoolean(DefaultAsCacheStrategy.AS_CALCULATION_CACHE_ENABLED_KEY, true)))
				.thenReturn(Boolean.TRUE);
		when(Boolean.valueOf(configuration.getBoolean(DefaultAsCacheStrategy.AS_MERGE_CACHE_ENABLED_KEY, true)))
				.thenReturn(Boolean.TRUE);
		...
		defaultAsCacheStrategy.setInvalidationTypeCodes(Collections.emptyList());
	}
	...
	@Test
	public void getWithMergeCacheEnabled()
	{
		// given
		when(Boolean.valueOf(configuration.getBoolean(DefaultAsCacheStrategy.AS_MERGE_CACHE_ENABLED_KEY, true)))
				.thenReturn(Boolean.TRUE);
		...
		final String cacheKeyFragment = "fragment";
		final AsCacheKey cacheKey = new DefaultAsCacheKey(AsCacheScope.MERGE, cacheKeyFragment);
	...
	@Test
	public void getWithMergeCacheDisabled()
	{
		// given
		when(Boolean.valueOf(configuration.getBoolean(DefaultAsCacheStrategy.AS_MERGE_CACHE_ENABLED_KEY, true)))
				.thenReturn(Boolean.FALSE);
