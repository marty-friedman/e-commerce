...
@UnitTest
public class AsFacetsReplaceMergeStrategyTest extends AbstractAsFacetsMergeStrategyTest
{
	private AsFacetsReplaceMergeStrategy mergeStrategy;

	@Before
	public void createMergeStrategy()
	{
		mergeStrategy = new AsFacetsReplaceMergeStrategy();
	...
	public void mergePromotedFacetsWithDuplicates()
	{
		...
		assertEquals(0, getTarget().getFacets().size());

		assertEquals(0, getTarget().getExcludedFacets().size());
	}
	...
	public void mergeFacetsWithDuplicates()
	{
		...
		// then
		assertEquals(0, getTarget().getPromotedFacets().size());
