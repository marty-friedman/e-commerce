...
public class DefaultPlatformRuleEngineServiceInitializeTest
{
	...
	public void setUp()
	{
		...
		when(releaseId.toExternalForm()).thenReturn("RELEASE_ID_EXTERNAL_FORM");
	...
	{
		final List<RuleEngineActionResult> results = service
				.initialize(singletonList(module), propagareToOtherNodes, false).waitForInitializationToFinish()
				.getResults();
	...
	private Thread createTenantAwareThread(final boolean propagateToOtherNodes)
	{
		...
		return new Thread(() -> ruleEngineKieModuleSwapper.switchKieModule(droolsModule,
