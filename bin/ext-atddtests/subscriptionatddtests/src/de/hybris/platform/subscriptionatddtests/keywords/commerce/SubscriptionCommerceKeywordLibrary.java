...
public class SubscriptionCommerceKeywordLibrary extends AbstractKeywordLibrary
{
	...
	public void setCartCurrencyTo(final String currency)
	{
		...
		subscriptionCartFacade.refreshProductXMLs();
	}
	...
	public void verifyClonedChildCarts(final String originalCartCode, final String clonedCartCode)
	{
		...
		else
		{
			...
			originalMasterCartModel.getChildren().forEach(
					originalChildCart -> clonedMasterCartModel.getChildren().forEach(
							clonedChildCart -> {
								if (originalChildCart.getBillingTime().getCode().equals(clonedChildCart.getBillingTime().getCode()))
								{
									verifyChildCartClone((CartModel) originalChildCart, (CartModel) clonedChildCart);
									assertEquals("Cloned child cart has wrong [parent] code.", clonedMasterCartModel.getCode(),
											clonedChildCart.getParent().getCode());
								}
							}));
		}
	}
