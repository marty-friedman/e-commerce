...
public class DefaultB2BCheckoutFacade extends DefaultCheckoutFacade implements CheckoutFacade
{
	...
	public CartData setCostCenterForCart(final String costCenterCode, final String orderCode)
	{
		final List<AbstractOrderModel> orders = getAbstractOrderGenericDao()
				.find(Collections.singletonMap(AbstractOrderModel.CODE, orderCode));
	...
	{
		if (order.getQuoteReference() != null)
		{
			order.setName(null);
			order.setDescription(null);
			order.setExpirationTime(null);
			order.setQuoteReference(null);
			order.setComments(null);
			for (final AbstractOrderEntryModel orderEntry : order.getEntries())
			{
				orderEntry.setComments(Collections.<CommentModel> emptyList());
			}
			getCommerceQuoteUtils().removeExistingQuoteDiscount(order);
		}
	}
	...
	@Override
	@Required
	public void setB2bPaymentTypeDataConverter(
			final Converter<CheckoutPaymentType, B2BPaymentTypeData> b2bPaymentTypeDataConverter)
	{
