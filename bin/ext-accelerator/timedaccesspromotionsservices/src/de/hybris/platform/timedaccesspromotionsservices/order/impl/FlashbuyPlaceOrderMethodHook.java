...
public class FlashbuyPlaceOrderMethodHook implements CommercePlaceOrderMethodHook, FlashbuyOrderCompleteStrategy
{
			...
			throws InvalidCartException
	{
		...
		{
			orderComplete(result.getOrder(), "B2C");
		}
		else if (TimedaccesspromotionsservicesConstants.ORDER_COMPLETE_STRATEGY_CNACC.equalsIgnoreCase(orderCompleteStrategy))
		{
			orderComplete(result.getOrder(), "CNACC");
		}

	}

	@Override
	public void orderComplete(final AbstractOrderModel order, final String str)
	{
		if ((str.equals("CNACC") && order.getPaymentStatus().equals(PaymentStatus.PAID)) || (str.equals("B2C")))
		{
