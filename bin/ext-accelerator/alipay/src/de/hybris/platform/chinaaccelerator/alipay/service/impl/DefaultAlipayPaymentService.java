			...
			final boolean isMobile)
	{
		...
		if (verify)
		{
			...
			if (notifyData.getRefund_status() != null)
			{
				if (LOG.isDebugEnabled())
				{
					LOG.debug("\nNotifyData Refund status is not null");
				}
			}
			else
			{ //TODO AlipayTradeStatus.TRADE_FINISHED returns for the Mobile payment, checking with Alipay on this at moment
				...
				{ //if TRADE_CLOSED or TRADE_SUCCESS
					if (LOG.isDebugEnabled())
					{
						LOG.debug("\n" + Calendar.getInstance().getTime() + " NotifyData Refund status is null");
					}
				...
				{ //if(tradeStatus.equals(AlipayTradeStatus.TRADE_FINISHED) || tradeStatus.equals(AlipayTradeStatus.WAIT_BUYER_PAY) ){
					...
					if (LOG.isDebugEnabled())
					{
						LOG.debug("\n NotifyData Refund status is null");
					}
				}
			}
		}
			...
			final String type)
	{
		...
		if (alipayTransaction != null) 
		{
			...
			catch (final IllegalArgumentException e) 
			{
				if (LOG.isDebugEnabled()) 
				{
					LOG.debug("No Enum const class AlipayTradeStatus of value of:"
							+ alipayTransaction.getLatestTradeStatus());
				}
			}
		}
