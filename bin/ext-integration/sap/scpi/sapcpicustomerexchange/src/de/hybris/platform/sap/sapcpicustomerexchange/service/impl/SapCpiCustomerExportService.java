...
public class SapCpiCustomerExportService extends CustomerExportService
{
	...
	/**
	 * @return the sapCpiCustomerService
	 */
	public SapCpiCustomerService getSapCpiCustomerService()
	{
		return sapCpiCustomerService;
	}

	/**
	 * @param sapCpiCustomerService
	 *           the sapCpiCustomerService to set
	 */
	@Required
	...
	{
		// ppopulte hci customer model
		final SapCpiCustomer customer = createCustomer(customerModel, baseStoreUid, sessionLanguage, addressModel);

		if (customer == null)
		{
			LOG.error("Could not set complete attributes for customer " + customerModel.getCustomerID());
			return;
		}

		getSapCpiCustomerService().createCustomer(customer).subscribe(err -> LOG.error(err.toString()),
				() -> LOG.info("Customer " + customer.getCustomerID() + " sent successfully."));
	}
