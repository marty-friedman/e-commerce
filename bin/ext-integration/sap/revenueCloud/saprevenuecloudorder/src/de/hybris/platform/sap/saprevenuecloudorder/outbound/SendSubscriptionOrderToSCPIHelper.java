...
import de.hybris.platform.sap.saprevenuecloudorder.actions.SendSubscriptionOrderAction;
...
public class SendSubscriptionOrderToSCPIHelper
{
	...
	public ResponseData sendOrder(final AbstractOrderModel order) {
		...
		final String pathUrl = getConfigurationService().getConfiguration()
				.getString(SaprevenuecloudorderConstants.SUBSCRIPTION_ORDER_PATH_URL);
		try {

			response = defaultCloudPlatformIntegrationConnection.sendPost(pathUrl, convertToJSON(subscriptionOrder));
