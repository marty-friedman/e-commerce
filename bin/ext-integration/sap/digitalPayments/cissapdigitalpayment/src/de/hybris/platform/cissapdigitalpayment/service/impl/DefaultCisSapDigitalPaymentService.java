...
public class DefaultCisSapDigitalPaymentService implements CisSapDigitalPaymentService
{
	 ...
	 * continuously polls the card with the session ID received during the card registration request. Polling will
	 * continue until it receives Cancelled, Timeout or Success status.
	 *
	 * @param sessionId
	...
	/**
	 *
	 * Used to configure the polling card interval. Read from the SAP Digital payment configuration. If no values are
	 * set, read the default value injected to defaultPollCardDelay property.
	 *
	 * @param sapDigiPayConfig
	 *           - SAP Digital payment configuration
	 *
