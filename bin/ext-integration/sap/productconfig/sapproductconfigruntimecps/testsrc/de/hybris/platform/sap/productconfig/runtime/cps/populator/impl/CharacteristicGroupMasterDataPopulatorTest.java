...
public class CharacteristicGroupMasterDataPopulatorTest
{
	...
	@Mock
	ConfigurationMasterDataService configurationMasterDataService;
	...
	public void initialize()
	{
		...
		Mockito.when(configurationMasterDataService.getGroupName(kbId, itemKey, itemTypeProduct, groupId)).thenReturn(groupName);
		Mockito.when(configurationMasterDataService.getGroupCharacteristicIDs(kbId, itemKey, itemTypeProduct, groupId))
				.thenReturn(csticIds);
	...
	{
		Mockito.when(configurationMasterDataService.getGroupName(kbId, itemKey, itemTypeProduct,
				SapproductconfigruntimecpsConstants.CPS_GENERAL_GROUP_ID)).thenReturn(groupName);
		Mockito.when(configurationMasterDataService.getGroupCharacteristicIDs(kbId, itemKey, itemTypeProduct,
	...
	public void testPopulateParentIsClass()
	{
		...
		Mockito.when(configurationMasterDataService.getGroupName(kbId, itemKey, itemTypeClassNode,
				SapproductconfigruntimecpsConstants.CPS_GENERAL_GROUP_ID)).thenReturn(groupName);
		Mockito.when(configurationMasterDataService.getGroupCharacteristicIDs(kbId, itemKey, itemTypeProduct,
				SapproductconfigruntimecpsConstants.CPS_GENERAL_GROUP_ID)).thenReturn(csticIds);
