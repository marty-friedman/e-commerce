...
public class ConfigurationMasterDataServiceImplTest
{
	...
	protected void fillProducts()
	{
		...
		productContainer.getCstics().get(CSTIC_ID).getPossibleValueSpecifics().put(POSSIBLE_VALUE_1,
				new CPSMasterDataPossibleValueSpecific());
		productContainer.getCstics().get(CSTIC_ID).getPossibleValueSpecifics().put(POSSIBLE_VALUE_2,
				new CPSMasterDataPossibleValueSpecific());
	...
	protected void fillCstics()
	{
		...
		csticContainer2.setType(ConfigurationMasterDataServiceImpl.CSTIC_TYPE_FLOAT);
	...
	{
		final String result = classUnderTest.getProductName(KB_ID, PRODUCT_ID);
		assertNotNull(result);
		Mockito.verify(accessService).getKbContainer(KB_ID, Locale.ENGLISH.getLanguage());
	...
	{
		assertTrue(classUnderTest.isProductMultilevel(KB_ID, PRODUCT_ID));
	}

	@Test(expected = IllegalStateException.class)
	public void testGetProductName_NoValidProductId()
	{
		classUnderTest.getProductName(KB_ID, "invalidProductId");
	}

	@Test
	public void testGetClassName()
	{
		final String result = classUnderTest.getClassName(KB_ID, CLASS_ID);
		assertNotNull(result);
		Mockito.verify(accessService).getKbContainer(KB_ID, Locale.ENGLISH.getLanguage());
		assertEquals(CLASS_NAME, result);
	}
	...
	{
		final CPSMasterDataClassContainer classContainer = classUnderTest.getClass(KB_ID, CLASS_ID);
	...
	{
		classUnderTest.getClass(KB_ID, INVALID_CLASS_ID);
	}

	@Test(expected = IllegalStateException.class)
	public void testGetClassName_NoValidClassId()
	{
		classUnderTest.getClassName(KB_ID, INVALID_CLASS_ID);
	}
	...
	{
		csticContainer.setType(ConfigurationMasterDataServiceImpl.CSTIC_TYPE_FLOAT);
	...
	{
		csticContainer.setType(ConfigurationMasterDataServiceImpl.CSTIC_TYPE_INTEGER);
		assertTrue(classUnderTest.isCsticNumericType(csticContainer));
	}
