...
public class PricingDocumentInputPopulatorTest
{
	...
	public void setup()
	{
		...
		Mockito.when(pricingItemInputConverter.convert(Mockito.any())).thenAnswer(invocation -> new PricingItemInput());
	...
	public void testfillPricingItemInput_NotMara()
	{
		...
		source.setRootItem(rootItems);
		classUnderTest.fillPricingItemsInput(source, pricingDocumentInput);
		Mockito.verify(pricingItemInputConverter, Mockito.times(0)).convert(Mockito.any());
	}
	...
	public void testfillPricingItemsInput_multiLevel()
	{
		...
		classUnderTest.fillPricingItemsInput(source, pricingDocumentInput);
		Mockito.verify(pricingItemInputConverter, Mockito.times(2)).convert(Mockito.any());
