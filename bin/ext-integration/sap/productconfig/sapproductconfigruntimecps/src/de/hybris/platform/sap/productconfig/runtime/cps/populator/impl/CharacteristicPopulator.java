 ...
 */
public class CharacteristicPopulator implements Populator<CPSCharacteristic, CsticModel>
{
	protected static final String INTERVAL_TYPE_UNCONSTRAINED = "0";
	private Converter<CPSValue, CsticValueModel> valueConverter;
	private Converter<CPSPossibleValue, CsticValueModel> possibleValueConverter;

	/**
	 * @return the valueConverter
	 */
	public Converter<CPSValue, CsticValueModel> getValueConverter()
	{
	 ...
	 */
	public void setValueConverter(final Converter<CPSValue, CsticValueModel> valueConverter)
	{
	 ...
	 */
	public Converter<CPSPossibleValue, CsticValueModel> getPossibleValueConverter()
	{
	 ...
	 */
	public void setPossibleValueConverter(final Converter<CPSPossibleValue, CsticValueModel> possibleValueConverter)
	{
	...
	protected void populateValues(final CPSCharacteristic source, final CsticModel target)
	{
		...
		if (source.getValues() != null)
		{
			...
			{
				final CsticValueModel valueModel = getValueConverter().convert(value);
	...
	protected void populatePossibleValues(final CPSCharacteristic source, final CsticModel target)
	{
		...
		if (source.getPossibleValues() != null)
		{
			hasOneInterval = fillPossibleValues(source, possibleValues);
		}
	...
	protected boolean fillPossibleValues(final CPSCharacteristic source, final List<CsticValueModel> possibleValues)
	{
		...
		for (final CPSPossibleValue possibleValue : source.getPossibleValues())
		{
			if (possibleValue.isSelectable() && !INTERVAL_TYPE_UNCONSTRAINED.equals(possibleValue.getIntervalType()))
			{
				...
				final CsticValueModel csticValueModel = getPossibleValueConverter().convert(possibleValue);
