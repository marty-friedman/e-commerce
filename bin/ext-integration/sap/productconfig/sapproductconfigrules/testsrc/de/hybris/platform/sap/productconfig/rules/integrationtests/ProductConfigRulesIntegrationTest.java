...
public class ProductConfigRulesIntegrationTest extends ProductConfigRulesTest
{
	...
	@Test
	public void testModifyDefaultConfigOfVariantDefaultMessage() throws CommerceCartModificationException,
			RuleEngineServiceException
	{
		...
		boolean softwareSelected = serviceConfigValueHelper.isValueAssigned(
				serviceConfigValueHelper.getCstic(config, CPQ_SOFTWARE), PAINTER);
	...
	@Test
	public void testModifyDefaultConfigAndRemoveAssignableValueBasedOnCartNoMessage() throws CommerceCartModificationException,
			RuleEngineServiceException
	{
		// For CPQ_LAPTOP
		// Set CPQ_DISPLAY = 17,
		...
		Assume.assumeFalse("Expected 'CPQ_DISPLAY' NOT to be '17', as rule should not match: ", display17Selected);
		Assume.assumeTrue("Expected 'CPQ_DISPLAY' to be '13' by default: ", display13Selected);
		Assume.assumeFalse("Expected 'CPQ_DISPLAY' NOT to be '15', as rule should not match: ", display15Selected);
		final Set<ProductConfigMessage> messages = config.getMessages();
		Assume.assumeTrue(messages.isEmpty());
	...
	public void testSetDisplayOnlyHideAnotherWarnMessage() throws RuleEngineServiceException
	{
		...
		Assume.assumeTrue("Default config: Estimated 'CPQ_SECURITY' Date should be visible", security.isVisible());
	...
	@Test
	public void testDoesNotHaveValueConditionAndDisplayMessageOnly() throws RuleEngineServiceException,
			CommerceCartModificationException
	{
		prepareAndPublishRule("cpq_test_doesNotHaveValue_condition_rule");

		ConfigModel config = ruleAwareService.createDefaultConfiguration(KB_CPQ_LAPTOP);
		Assume.assumeTrue(StringUtils.isEmpty(serviceConfigValueHelper.getCstic(config, CPQ_DISPLAY).getName()));
		assertSingleMessage(config, ProductConfigMessageSeverity.INFO, expectedMessage);
	...
	public void testShowMessagesOnProductAndCsticLevels() throws CommerceCartModificationException, RuleEngineServiceException
	{
		...
		Assume.assumeTrue(StringUtils.isEmpty(displayCstic.getName()));
		assertNoMessage(config);
	...
	public void testSupportMultilevelConfiguration() throws CommerceCartModificationException, RuleEngineServiceException
	{
		...
		final CsticModel surroundMode = serviceConfigValueHelper.getCstic(config, CPQ_HT_SURROUND_MODE);
		Assume.assumeTrue(StringUtils.isEmpty(surroundMode.getName()));
		...
		assertNotNull("frontSpeakerModel should e not null: ", frontSpeakerModel);
		...
		assertNull("frontSpeakerColor should be null: ", frontSpeakerColor);
		...
		assertNotNull("rearSpeakerColor should not be null: ", rearSpeakerColor);
		...
		assertNotNull("rearSpeakerColor should not be null: ", frontSpeakerColor);

		//Get rear speaker model
		rearSpeakerModel = serviceConfigValueHelper.getCstic(config, "4", CPQ_HT_SPK_MODEL);
		assertNull("rearSpeakerModel should not be null: ", rearSpeakerModel);
		//Get rear speaker color
		rearSpeakerColor = serviceConfigValueHelper.getCstic(config, "4", CPQ_HT_SPK_COLOR);
		assertNull("rearSpeakerColor should not be null: ", rearSpeakerColor);
	}
	 ...
	 */
	@Test
	public void testDefaultMessageNotShownWhenDefaultValueModified() throws CommerceCartModificationException,
			RuleEngineServiceException
	{
		...
		config = ruleAwareService.createDefaultConfiguration(KB_CPQ_LAPTOP);
		displayCstic = serviceConfigValueHelper.getCstic(config, CPQ_DISPLAY);
		assertEquals("'CPQ_DISPLAY' should changed from '13' -> '17': ", CPQ_DISPLAY_17, displayCstic.getAssignedValues().get(0)
				.getName());
		assertNoMessage(config);
	}
	...
	public void testRulesAreAppliedOnCreateFromExternal() throws CommerceCartModificationException
	{
		...
		// check
		displayCstic = serviceConfigValueHelper.getCstic(config, CPQ_DISPLAY);
		assertEquals("Rules changing default values, should not be executed on create from external", CPQ_DISPLAY_13, displayCstic
				.getAssignedValues().get(0).getName());
		assertSingleMessage(config, ProductConfigMessageSeverity.WARNING, expectedMessage);
	}
}
