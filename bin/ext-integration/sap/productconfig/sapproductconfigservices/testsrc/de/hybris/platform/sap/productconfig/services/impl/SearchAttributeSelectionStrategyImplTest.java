...
public class SearchAttributeSelectionStrategyImplTest
{
	...
	public void setup() throws NoValidSolrConfigException
	{
		...
		Mockito.when(sessionAccessService.getSolrIndexedProperties()).thenReturn(indexedProperties);
		Mockito.when(solrFacetSearchConfigSelectionStrategy.getCurrentSolrFacetSearchConfig()).thenReturn(searchConfig);
		Mockito.when(searchConfig.getSolrIndexedTypes()).thenReturn(indexedTypes);
		Mockito.when(indexedType.getSolrIndexedProperties()).thenReturn(indexedPropertyList);
	...
	{
		final boolean available = classUnderTest.isAttributeAvailableOnSearchIndex(propertyName);
	...
	{
		final boolean available = classUnderTest.isAttributeAvailableOnSearchIndex(propertyUnknown);
	...
	{
		classUnderTest.isAttributeAvailableOnSearchIndex(null);
	}
	...
	{
		Mockito.when(solrFacetSearchConfigSelectionStrategy.getCurrentSolrFacetSearchConfig()).thenReturn(null);
	...
	{
		Mockito.when(searchConfig.getSolrIndexedTypes()).thenReturn(null);
		classUnderTest.compileIndexedProperties();
	}
}
