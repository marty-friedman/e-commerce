...
public class CartConfigurationPopulatorTest extends AbstractOrderConfigurationPopulatorTest
{
	...
	public void setup()
	{
		...
		Mockito
				.when(configurationPricingOrderIntegrationService.ensureConfigurationInSession(Mockito.any(), Mockito.any(),
						Mockito.any())).thenReturn(configModel);
		Mockito.when(sourceEntry.getOrder()).thenReturn(source);
		Mockito.when(sourceEntry.getBasePrice()).thenReturn(basePrice);
		Mockito.when(
				priceDataFactory.create(PriceDataType.BUY, BigDecimal.valueOf(basePrice), sourceEntry.getOrder().getCurrency()))
	...
	public void testPopulateNoTargetItems()
	{
		...
		Mockito.when(cpqConfigurableChecker.isCPQConfigurableProduct(any())).thenReturn(true);
	...
	public void testValidatePriceError()
	{
		...
		Mockito.verify(configurationPricingOrderIntegrationService).updateCartEntryPrices(sourceEntry, false, null);
	}
	...
	public void testValidatePriceResultTrue()
	{
		...
		targetEntry.setConfigurationConsistent(true);
		targetEntry.setConfigurationInfos(null);
		Mockito.when(configurationPricingOrderIntegrationService.updateCartEntryPrices(sourceEntry, false, null)).thenReturn(
				Boolean.TRUE);
	...
	private PK initializeSourceItem(final String extConfig, final String configId)
	{
		...
		Mockito.when(cpqConfigurableChecker.isCPQConfigurableProduct(any())).thenReturn(configId != null);
	...
	{
		classUnderTest.rePopulateCart(sourceCart, targetCart);
		Mockito.verify(cartPopulator).populate(sourceCart, targetCart);
	}

	@Test
	public void testCalculateCart()
	{
		classUnderTest.calculateCart(sourceCart);
