...
public class UiTypeFinderImpl implements UiTypeFinder
{
	...
	protected boolean isReadonly(final CsticModel model, final boolean isDebugEnabled)
	{
		...
		if (model.isConstrained() && (model.getAssignableValues() == null || model.getAssignableValues().isEmpty()))
		{
			isReadOnly = true;
		}
	...
	{
		boolean isSingleSelectionImage;
		isSingleSelectionImage = isSingleSelection(model, isDebugEnabled) && !model.isAllowsAdditionalValues() && hasValueImages;
	...
	{
		boolean isCheckbox;
		isCheckbox = isMultiSelection(model, isDebugEnabled) && model.getStaticDomainLength() == 1 && !hasValueImages;
	...
	{
		boolean isMultiSelectionImage;
		isMultiSelectionImage = isMultiSelection(model, isDebugEnabled) && !model.isAllowsAdditionalValues() && hasValueImages;
	...
	protected boolean isInput(final CsticModel model, final boolean isDebugEnabled)
	{
		...
		if (isValueTypeSupported(model, isDebugEnabled) && !model.isReadonly() && !model.isMultivalued())
		{
			if (model.getAssignableValues().isEmpty())
			{
			...
			{
				if (model.isIntervalInDomain() && !model.isAllowsAdditionalValues())
				{
					isInput = true;
				}
			}
