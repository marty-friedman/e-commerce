...
public class ClassificationSystemCPQAttributesProviderImpl implements ClassificationSystemCPQAttributesProvider
{
	...
	private static final String HYBRIS_NAME = "HYBRIS_NAME";
	private static final String MODEL_LONGTEXT = "MODEL_LONGTEXT";
			...
			final ClassificationSystemVersionModel systemVersion)
	{
		...
		else
		{
			...
			cpqAttributes = new ClassificationSystemCPQAttributesContainer(code, name, description, valueNames, csticMedia,
					csticValueMedia);
		}
		return cpqAttributes;
	}
			...
			final boolean isDebugEnabled)
	{
		...
		if (!StringUtils.isEmpty(hybrisLongText))
		{
			...
			logValue(CSTIC_MODEL, CSTIC_NAME, model.getName(), LONG_TEXT, HYBRIS_NAME, longText, isDebugEnabled);
		}
		else if (!StringUtils.isEmpty(model.getLongText()))
		{
			longText = model.getLongText();
			logValue(CSTIC_MODEL, CSTIC_NAME, model.getName(), LONG_TEXT, MODEL_LONGTEXT, longText, isDebugEnabled);
		}
		else
		{
			longText = null;
			...
			final boolean isDebugEnabled)
	{
		...
		if (!StringUtils.isEmpty(hybrisName))
		{
			...
			logValue(CSTIC_MODEL, CSTIC_NAME, csticModel.getName(), DISPLAY_NAME, HYBRIS_NAME, displayName, isDebugEnabled);
		}
		...
		else
		{
			...
			logValue(CSTIC_MODEL, CSTIC_NAME, csticModel.getName(), DISPLAY_NAME, MODEL_NAME, displayName, isDebugEnabled);
		}
	...
	/**
	 * Retrieves the hybris classification attribute for a characteristic name (which is part of a CPQ configuraton
	 * model). <br>
	 *
	 * @param name
			...
			final ClassificationSystemCPQAttributesContainer cpqAttributes, final boolean isDebugEnabled)
	{
		...
		if (displayName == null)
		{
			...
			logValue(CSTIC_VALUE_MODEL, VALUE_NAME, valueModel.getName(), DISPLAY_NAME, MODEL_NAME, displayName, isDebugEnabled);
		}
		return displayName;
	}
			...
			final ClassificationSystemCPQAttributesContainer cpqAttributes, final boolean isDebugEnabled)
	{
		...
		if (!StringUtils.isEmpty(hybrisValueName))
		{
			...
			logValue(CSTIC_VALUE_MODEL, VALUE_NAME, valueModel.getName(), DISPLAY_NAME, HYBRIS_NAME, displayName, isDebugEnabled);
		}
		else if (!StringUtils.isEmpty(langDepName))
		{
			displayName = langDepName;
	 ...
	 * <br>
	 * If at least one system is available: Returns the first one which contains '300' as part of its name, as this
	 * matches the customizing proposal for SAP Integration. '300' stands for class type 300 in ERP, which is typically
	 * used for product configuration. <br>
	 * <br>
