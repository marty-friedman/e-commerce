...
public class VariantOverviewController extends AbstractConfigurationOverviewController
{
			...
			final HttpServletRequest request) throws CMSItemNotFoundException
	{
		...
		{
			LOGGER.debug(
					CONFIG_GET_RECEIVED_FOR + productCode + CURRENT_SESSION + getSessionAccessFacade().getSessionId() + CLOSING_QUOTE);
		}
	...
	public void addVariantToCartCleanUp(@PathVariable("productCode") final String productCodeEncoded)
	{
		...
		if (LOGGER.isDebugEnabled())
		{
			LOGGER.debug("AddVariantToCart GET received for '" + productCode + CURRENT_SESSION
					+ getSessionAccessFacade().getSessionId() + CLOSING_QUOTE);
		}
			...
			final ConfigurationOverviewData configOverviewData)
	{
		...
		configOverview = getConfigurationOverviewFacade().getOverviewForProductVariant(productCode, configOverviewData);
	 ...
	 *           overviewUIData containing the product code of the variant
	 * @param model
	 *           view model
	 * @param request
	 *           http request
