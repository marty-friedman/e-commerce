...
public class ConfigCopyCartEntryActionHandler implements CartEntryActionHandler
{
	...
	protected ConfigurationData copyConfiguration(final OrderEntryData entryToCopy)
	{
		...
		final String newConfigId = getConfigCartFacade().copyConfiguration(configId, pCode);
		if (LOG.isDebugEnabled())
		{
			LOG.debug("copied cofguration session with id '" + configId + "' and created new session with id '" + newConfigId + "'");
		}
	...
	{
		return getSessionAccessFacade().getConfigIdForCartEntry(entryToCopy.getItemPK());
	}
	...
	{
		final boolean configurable = getCpqConfigurableChecker().isCPQConfigurableProduct(cartEntry.getProduct());
		if (LOG.isDebugEnabled())
		{
			if (configurable)
			{
