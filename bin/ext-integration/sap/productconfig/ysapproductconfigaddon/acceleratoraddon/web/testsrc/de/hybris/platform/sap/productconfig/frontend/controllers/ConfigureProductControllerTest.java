...
@UnitTest
public class ConfigureProductControllerTest extends AbstractProductConfigControllerBaseTest
{
	...
	public void testUpdateConfigureProductRedirectIsCorrect() throws Exception
	{
		...
		final String forward = classUnderTest.configureProduct(PRODUCT_CODE, model, request, redirectAttributes);
	...
	public void testBreadcrumbIsSet() throws Exception
	{
		...
		request.setAttribute("de.hybris.platform.acceleratorcms.utils.SpringHelper.bean.requestContextData",
				new RequestContextData());
		classUnderTest.configureProduct(PRODUCT_CODE, model, request, redirectAttributes);
	...
	public void testCreateKBKey() throws Exception
	{
		...
		final KBKeyData actualKBKey = classUnderTest.createKBKeyForProduct(productData);
	...
	{
		given(sessionAccessFacade.getCartEntryForProduct(PRODUCT_CODE)).willReturn("4711");
	...
	@Test
	public void testCleanUpSessionAttributeWithAttributeNotExist()
	{
		given(sessionAccessFacade.getCartEntryForProduct(PRODUCT_CODE)).willReturn(null);

		classUnderTest.cleanUpSessionAttribute(PRODUCT_CODE);

		verify(sessionAccessFacade, times(0)).removeCartEntryForProduct(PRODUCT_CODE);
	}
