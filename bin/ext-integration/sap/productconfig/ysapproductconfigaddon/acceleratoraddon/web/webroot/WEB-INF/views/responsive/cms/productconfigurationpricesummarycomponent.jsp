...
<c:set var="variantOverview" value="${not empty overviewUiData and overviewUiData.overviewMode eq 'VARIANT_OVERVIEW'}"/>
...
<div id="priceSummary" class="cpq-price-sum col-xs-12 col-sm-6 col-sm-offset-6">
	<c:if test="${showPriceDetails and !config.priceSummaryCollapsed and noneVariantOverview}">
			<div id="priceSummarySubContent" class="cpq-price-sum-sub">
				...
				<div id="basePriceValue" class="cpq-price-value" aria-describedby="basePriceLabel">${config.pricing.basePrice.formattedValue}</div>
				...
				<div id="selectedOptionsValue" class="cpq-price-value" aria-describedby="selectedOptionsLabel">${config.pricing.selectedOptions.formattedValue}</div>
			</div>
	</c:if>
	<div class="cpq-price-sum-total">
		...
		<c:choose>
			<c:when test="${eitherOneorAnotherVariantOverview}">
				<div id="currentTotalValue" class="cpq-price-value cpq-price-total" aria-describedby="priceTotalLabel">${product.price.formattedValue}</div>
			</c:when>
			<c:otherwise>
				<div id="currentTotalValue" class="cpq-price-value cpq-price-total" aria-describedby="priceTotalLabel">${config.pricing.currentTotal.formattedValue}</div>
			</c:otherwise>
		</c:choose>
