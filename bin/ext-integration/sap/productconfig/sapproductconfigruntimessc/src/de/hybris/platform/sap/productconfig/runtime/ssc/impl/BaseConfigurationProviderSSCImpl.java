...
public abstract class BaseConfigurationProviderSSCImpl implements ConfigurationProvider
{
	...
	protected IKnowledgeBaseData getKbByDate(final String productCode, final Date kbDate)
	{
		final String kbDateString = kbDateFormat.get().format(kbDate);
		...
		try
		{
			...
			timer.start(FIND_KB_TIMER_START_STRING);
			kbVersions = configSession.findKnowledgeBases(TYPE_MARA, productCode, null, kbDateString, null, null, null, null, true);
			timer.stop();
		}
		catch (final IpcCommandException e)
		{
			LOG.error(CANNOT_FIND_KNOWLEDGE_BASE_VERSION_FOR_PRODUCT + productCode + "] and Date[" + kbDate + "]", e);
		}
	...
	{
		return null != getKbByDate(productCode, kbDate);
	}
	...
	{
		return null != getKbByVersion(kbKey, externalConfig);
	}


	protected IKnowledgeBaseData getKbByVersion(final KBKey kbKey, final String externalConfig)
	{
		IKnowledgeBaseData[] kbVersions = null;
		...
		{
			final String configurationOnly = extractConfigurationFromXml(externalConfig);
			final ExternalConfigurationParser parser = new ExternalConfigurationParser();
			parser.parse(configurationOnly);
			final String kbNameFromExternalConfiguration = parser.get_kb_name();
			final String kbVesionFromExternalConfiguration = parser.get_kb_version();

			try
			{
				final IConfigSession configSession = createSession(kbKey);
				timer.start(FIND_KB_TIMER_START_STRING);
				kbVersions = configSession.findKnowledgeBases(TYPE_MARA, productCode, null, null, null, null, null, null, false);
				timer.stop();
			}
			catch (final IpcCommandException e)
			{
				LOG.error(CANNOT_FIND_KNOWLEDGE_BASE_VERSION_FOR_PRODUCT + productCode + "]", e);
			}
			return extractKbVersion(kbNameFromExternalConfiguration, kbVesionFromExternalConfiguration, kbVersions);
		}
		return null;
	}

	protected IKnowledgeBaseData extractKbVersion(final String kbNameFromExternalConfiguration,
			final String kbVesionFromExternalConfiguration, final IKnowledgeBaseData[] kbVersions)
	{
		if (kbVersions != null)
		{

			for (final IKnowledgeBaseData kbdEntry : kbVersions)
			{
				if (kbdEntry.getKbName().equals(kbNameFromExternalConfiguration)
						&& kbdEntry.getKbVersion().equals(kbVesionFromExternalConfiguration))
				{
					return kbdEntry;
				}
			}
