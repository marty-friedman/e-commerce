...
public class ProductConfigurationStrategyImplTest extends JCORecTestBase
{
	...
	{
		classUnderTest = genericFactory.getBean(SapproductconfigsombolConstants.ALIAS_BEAN_PCFG_STRATEGY);
		...
		EasyMock.expect(exportParametersRead.getTable(ProductConfigurationStrategyImpl.TABLE_ET_VCFG_INST))
				.andReturn(instancesTableRead);
		EasyMock.expect(exportParametersRead.getTable(ProductConfigurationStrategyImpl.TABLE_ET_VCFG_CHAR))
				.andReturn(csticsTableRead);
	...
	public void testCreateConfigModels()
	{
		...
		final CPQItem itemAfterReadingCfg = (CPQItem) salesDoc.getItemList().get(0);
		assertNotNull(itemAfterReadingCfg);
		assertNotNull(itemAfterReadingCfg.getExternalConfiguration());
		assertEquals(createdAt, itemAfterReadingCfg.getKbDate());
	}
	...
	public void testRefChars()
	{
		...
		assertEquals("We don't expect context attributes as we ignore JCO ref char output", 0, contextAttributes.size());
	}
