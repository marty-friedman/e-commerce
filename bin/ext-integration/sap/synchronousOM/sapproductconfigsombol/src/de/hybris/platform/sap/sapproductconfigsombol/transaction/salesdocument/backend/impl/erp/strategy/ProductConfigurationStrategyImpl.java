...
public class ProductConfigurationStrategyImpl implements ProductConfigurationStrategy
{
	...
	{
		sapLogger.debug("Cstic will be sent: " + checkAuthorValid(csticValue.getAuthorExternal() + " : " + instance.getId() + ", "
				+ characteristic.getName() + ", " + csticValue.getName() + ", " + csticValue.getAuthor()));
			...
			final List<String> configurableItems)
	{
		try
		{
			...
			createConfigModels(configurableItems, salesDoc, instanceTable, characteristicTable,
					function.getTableParameterList().getTable(TABLE_TT_REFCHAR));
			...
			final JCoTable csticsTableRead, final JCoTable refCharTableRead)
	{
		...
		//We don't set context attributes as this is not needed with the current capabilities (no configuration change).
		//(One of our engines cannot handle multi-values context attributes like SDCOM-VKOND, which do not influence configuration but are still
		//provided by LO-API).
		//This is why refCharTableRead is not handled
	...
	protected void createCharacteristics(final String handle, final Configuration configuration, final JCoTable csticsTableRead)
	{
		...
		do
		{
			...
			if (currentHandle.equals(handle))
			{
				...
				newCharacteristic.setInvisible(("X").equals(csticsTableRead.getString("ATVIE")));
			...
			final JCoTable instancesTableRead, final JCoTable csticsTableRead, final JCoTable refCharTableRead)
	{
		for (final String itemHandle : configurableItems)
		{
			...
			final CPQItem item = (CPQItem) salesDoc.getItem(new TechKey(itemHandle));
			item.setExternalConfiguration(configuration);
			item.setKbDate(salesDoc.getHeader().getCreatedAt());
		}
	}
	...
	protected String mapObjectCategory(final String objectCategory)
	{
		...
		{
			sapLogger.log(Severity.ERROR, LogCategories.APPLICATIONS,
					"Unknow object category in reading configuration: " + objectCategory);
		}
		return result;
	...
	protected void createContextAttributes(final String handle, final Configuration configuration, final JCoTable refCharTableRead)
	{
		...
		do
		{
			retriveFirstRowValue(handle, configuration, refCharTableRead, characteristicValues);
		}
	 ...
	 */
	private void retriveFirstRowValue(final String handle, final Configuration configuration, final JCoTable refCharTableRead,
			final List<CharacteristicValue> characteristicValues)
	{
		if (handle.equals(refCharTableRead.getString("HANDLE_ITEM")))
		{
			...
			if (!Collections.isEmpty(valuesForContextAttribute))
			{
	...
	{
		final KBKey kbKey = new KBKeyImpl(configuration.getRootInstance().getObjectKey(), null, null, null,
				salesDoc.getHeader().getCreatedAt());
