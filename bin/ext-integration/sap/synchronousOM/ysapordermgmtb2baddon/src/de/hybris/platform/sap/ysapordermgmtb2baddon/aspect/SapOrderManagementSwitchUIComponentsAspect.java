...
public class SapOrderManagementSwitchUIComponentsAspect
{
	...
	public Object applyUIChanges(final ProceedingJoinPoint pjp) throws Throwable
	{
		...
		final String validationsAttribute = "validations";
		boolean prodSubsMessage = false;
		if ("pages/cart/cartPage".equals(uiComponent) && null != getSessionService().getAttribute(validationsAttribute))
		{
			final Model model = (Model) pjp.getArgs()[0];
			model.addAttribute("validationData", getSessionService().getAttribute(validationsAttribute));
			getSessionService().setAttribute(validationsAttribute, null);
			pjp.getArgs()[0] = model;
		}
		else if ("fragments/cart/addToCartPopup".equals(uiComponent)
				&& null != getSessionService().getAttribute(validationsAttribute))
		{
