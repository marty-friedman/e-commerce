...
public class MessageMappingRulesContainerImpl implements MessageMappingRulesContainer
{
	...
	static public class Key
	{
		...
		public int hashCode()
		{
			return 1 + MessageMappingRule.hashCodeField(id) * 0x1F + MessageMappingRule.hashCodeField(number)
					+ 0x7 * MessageMappingRule.hashCodeField(severity);
		}
	...
	private static final Logger LOG = Logger.getLogger(MessageMappingRulesContainerImpl.class.getName());
	...
	{
		LOG.debug("mostNarrow(String beClass, String beNumber, String beSeverity)");
			...
			if (rulesList != null)
			{
				for (final MessageMappingRule rule : rulesList)
				{
							...
							beMes.getBeV2(), beMes.getBeV3(), beMes.getBeV4()))
					{
						...
						{
							LOG.debug("found rule " + rule + " for " + beMes.getBeClass() + " , " + beMes.getBeNumber() + " , "
									+ beMes.getBeSeverity());
						}
						return rule;
					}
		...
		LOG.info("Found no message mapping rule for backend message: " + beMes.getBeClass() + " , " + beMes.getBeNumber() + " , "
