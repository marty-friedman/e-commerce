...
// Suppressing to fix this it required complex change.
public abstract class SalesDocumentERP extends IsaBackendBusinessObjectBaseSalesERP implements SalesDocumentBackend
{
	 ...
	 *
	 */
	public static final String RESOURCE_KEY = "sapordermgmt.erp.ui.rfc.messages.label.v2167";

	private LrdCloseStrategy closeStrategy;
	private List<SalesDocumentERPHook> salesDocumentERPHooks;
	...
	protected Map<String, String> itemVariantMap = new HashMap<String, String>(0);
			...
			final TransactionConfiguration transConf) throws BackendException
	{
		...
		if ((salesDocument.getMessageList().size() > 0) && (salesDocument.getItemList().size() > 0))
		{
			for (final Message cartInfo : salesDocument.getMessageList())
			{
				//Don't add messages that are assigned explicitly to checkout
				arglst = setArgumentList(arglst, cartInfo);
			}
		}

		clearMessagesFromSalesDocument(salesDocument, arglst);


		// remove item from buffer if it available
		removeBufferItem(salesDocument, itemsToDelete);

		// Try to delete the items
		deleteItem(salesDocument, itemsToDelete);
	}

	/**
	 * @param salesDocument
	 * @param arglst
	 * @param isProductSubsMsg
	 */
	private void clearMessagesFromSalesDocument(final SalesDocument salesDocument, final List<String> arglst)
	{
		if (arglst == null)
		{
		...
		else
		{
			...
			{
				if (!(arglst.contains(item.getProductId())))
				{
					salesDocument.clearMessages();
				}
			}
	 ...
	 * @return
	 */
	private List<String> setArgumentList(List<String> arglst, final Message cartInfo)
	{
		if (cartInfo instanceof OrderMgmtMessage)
		{
			final OrderMgmtMessage orderMgmtMessage = (OrderMgmtMessage) cartInfo;
			if (RESOURCE_KEY.equals(orderMgmtMessage.getResourceKey()))
			{
	 ...
	 * @throws BackendException
	 */
	private void deleteItem(final SalesDocument salesDocument, final TechKey[] itemsToDelete) throws BackendException
	{
		final JCoConnection aJCoCon = getDefaultJCoConnection();
	 ...
	 * @param itemsToDelete
	 */
	private void removeBufferItem(final SalesDocument salesDocument, final TechKey[] itemsToDelete)
	{
		for (final TechKey element : itemsToDelete)
		{
			...
			final boolean directRead) throws BackendException
	{
		...
		GetAllReadParameters.headerCondTypeFreight = headerCondTypeFreight;
		GetAllReadParameters.setIpcPriceAttributes = setIpcPriceAttributes;
		GetAllReadParameters.isIncompletionLogRequested = isIncompletionLogRequested;
		...
		afterReadFromBackend(salesDocument, aJCoCon);
	}
	...
	private void readItemsFromLOAPI(final SalesDocument salesDocument, final JCoConnection aJCoCon) throws BackendException
	{
	...
	public void updateInBackend(final SalesDocument salesDocument, final TransactionConfiguration shop) throws BackendException
	{
		...
		OrderMgmtMessage orderMgmtMessagesave = null;
		...
		if (salesDocument.getMessageList().size() > 0 && salesDocument.getItemList().size() > 0)
		{
			for (final Message cartInfo : salesDocument.getMessageList())
			{
				...
				{
					final OrderMgmtMessage orderMgmtMessage = (OrderMgmtMessage) cartInfo;
					orderMgmtMessagesave = findOrderMgmtMessageToBeSaved(orderMgmtMessagesave, salesDocument, orderMgmtMessage);
				}
			}
		}
		else if (salesDocument.getMessageList().size() > 0)
		{
			for (final Message cartInfo : salesDocument.getMessageList())
			{
				//Don't add messages that are assigned explicitly to checkout
				clearMessagesFromSalesDocument(salesDocument, cartInfo);
			}
		}

		final BackendCallResult result = setStrategy.execute(this, salesDocument, itemBuffer.getItemsERPState(), shop, aJCoCon,
				null, false);
		if (orderMgmtMessagesave != null)
		{
			salesDocument.addMessage(orderMgmtMessagesave);
		}
		...
		checkShippingConditionsIfNotErroneous(salesDocument);

		afterWriteDocument(salesDocument, aJCoCon);
	 ...
	 * @param aJCoCon
	 */
	protected void afterWriteDocument(final SalesDocument salesDocument, final JCoConnection aJCoCon)
	{
		if (getSalesDocumentERPHooks() != null)
		{
			for (final SalesDocumentERPHook salesDocumentERPHook : getSalesDocumentERPHooks())
			{
				salesDocumentERPHook.afterWriteDocument(salesDocument, aJCoCon);
			}
		}
	}

	protected void afterReadFromBackend(final SalesDocument salesDocument, final JCoConnection aJCoCon)
	{
		if (getSalesDocumentERPHooks() != null)
		{
			for (final SalesDocumentERPHook salesDocumentERPHook : getSalesDocumentERPHooks())
			{
				salesDocumentERPHook.afterReadFromBackend(salesDocument, aJCoCon);
			}
		}
	}
	 ...
	 * @throws BackendException
	 */
	private void checkShippingConditionsIfNotErroneous(final SalesDocument salesDocument) throws BackendException
	{
		if (!isErroneous())
		{
	...
	/**
	 * @param orderMgmtMessagesave
	 * @param isInfoMsgsave
	 * @param orderMgmtMessage
	 * @return
	 */
	private OrderMgmtMessage findOrderMgmtMessageToBeSaved(OrderMgmtMessage orderMgmtMessagesave,
			final SalesDocument salesDocument, final OrderMgmtMessage orderMgmtMessage)
	{
		List<String> arglst = null;
		if (RESOURCE_KEY.equals(orderMgmtMessage.getResourceKey()))
		{
			for (final Item item : salesDocument.getItemList())
			{
				...
				{
					orderMgmtMessagesave = orderMgmtMessage;
				}

			}
		}
		return orderMgmtMessagesave;
	}
	 ...
	 * @param cartInfo
	 */
	private void clearMessagesFromSalesDocument(final SalesDocument salesDocument, final Message cartInfo)
	{
		if (cartInfo instanceof OrderMgmtMessage)
		{
			final OrderMgmtMessage orderMgmtMessage = (OrderMgmtMessage) cartInfo;
			if (RESOURCE_KEY.equals(orderMgmtMessage.getResourceKey()))
			{
				salesDocument.clearMessages(orderMgmtMessage.getResourceKey());
			}
		}
	}
	...
	/**
	 * @return the salesDocumentERPHooks
	 */
	public List<SalesDocumentERPHook> getSalesDocumentERPHooks()
	{
		return salesDocumentERPHooks;
	}

	/**
	 * @param salesDocumentERPHooks
	 *           the salesDocumentERPHooks to set
	 */
	public void setSalesDocumentERPHooks(final List<SalesDocumentERPHook> salesDocumentERPHooks)
	{
		this.salesDocumentERPHooks = salesDocumentERPHooks;
	}
