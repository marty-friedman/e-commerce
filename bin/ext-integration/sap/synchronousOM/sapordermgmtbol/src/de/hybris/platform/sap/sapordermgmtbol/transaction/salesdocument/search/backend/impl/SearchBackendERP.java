...
public class SearchBackendERP extends BackendBusinessObjectBaseJCo implements SearchBackend
{
	...
	private static final Log4JWrapper sapLogger = Log4JWrapper.getInstance(SearchBackendERP.class.getName());
	...
	public int getDateRangeInDays()
	{
		...
		{
			dateRangeInDays = Integer.valueOf(
					(String) moduleConfigurationAccess.getProperty(SapordermgmtbolConstants.CONFIGURATION_PROPERTY_SEARCH_DATE_RANGE));
		}
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.hybris.platform.sap.sapordermgmtbol.transaction.salesdocument.search
	 * .backend.interf.SearchBackend#getSearchResult ()
	 */
	@Override
	public SearchResultList getSearchResult(final SearchFilter searchFilter) throws BackendException
	{
		final JCoConnection connection = getDefaultJCoConnection();
		final JCoFunction function = connection.getFunction(ERP_ISA_GEN_DOCUMENT_SEL);
		...
		fillSelectionOptions(tableParameterList.getTable("IT_SEL_OPT"), searchFilter);
	...
	protected SearchResultList processResults(final JCoTable table)
	{
		...
		if (sapLogger.isDebugEnabled())
		{
			...
			debugOutput.append("number of results: ").append(resultList.size());
	 ...
	 * @param maxHits
	 */
	protected void performLogging(final SearchResultList resultList, final int maxHits)
	{
		if (isErrorStatus(resultList, maxHits))
		{
			sapLogger.log(Severity.ERROR, LogCategories.APPLICATIONS, "Result list size exceeds maximum number of hits");
		}
		else if (isWarningStatus(resultList, maxHits))
		{
			sapLogger.log(Severity.WARNING, LogCategories.APPLICATIONS,
					"Result list size exceeds 90% of the maximum number of hits");
		}
	}

	private boolean fillSearchResult(final SearchResult searchResult, final JCoTable table)
	{

		searchResult.setKey(new TechKey(table.getString("VALUE")));
		tryNextRow(table);
		searchResult.setCreationDate(ConversionHelper.convertDateStringToDate(table.getString("VALUE")));
		tryNextRow(table);
	...
	private void tryNextRow(final JCoTable table)
	{
		if (!table.nextRow())
		{
			throw new ApplicationBaseRuntimeException("No search results; search meta data has been provided inconsistently");
		}
	}
	...
	{
		final SearchResult result = (SearchResult) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_SEARCH_RESULT_ENTRY);
		return result;
	}

	/**
	 * Fills search result attributes for the call of function module {@link SearchBackendERP#ERP_ISA_GEN_DOCUMENT_SEL}
	 *
	 * @param table
	...
	/**
	 * Fills selection options for the call of function module {@link SearchBackendERP#ERP_ISA_GEN_DOCUMENT_SEL}
	 *
	 * @param table
	 ...
	 *           Filter attributes
	 */
	protected void fillSelectionOptions(final JCoTable table, final SearchFilter searchFilter)
	{
		if (sapLogger.isDebugEnabled())
		{
			sapLogger.debug("Search filter, shipping status: " + searchFilter.getShippingStatus() + ", product ID: "
		...
		{
			throw new ApplicationBaseRuntimeException("It was tried to execute a sales order search without sold to id.");
		}
	 ...
	 * @param productId
	 */
	private void fillSelectProductId(final JCoTable table, final String productId)
	{
		addNewEntryToSelectionTable(table);
	...
	private void fillSelectShippingStatus(final JCoTable table, final String shippingStatus)
	{
		addNewEntryToSelectionTable(table);
	...
	/**
	 * Fills counter options for the call of function module {@link SearchBackendERP#ERP_ISA_GEN_DOCUMENT_SEL}
	 *
	 * @param table
	...
	protected int getMaxHits()
	{
		if (maxHits == null)
		{
			maxHits = Integer.valueOf(
					(String) moduleConfigurationAccess.getProperty(SapordermgmtbolConstants.CONFIGURATION_PROPERTY_SEARCH_MAX_HITS));
		}
	...
	protected boolean isWarningStatus(final SearchResultList results, final int maxHits)
	{
		final double threshold = 0.9;
		return exceedsThreshold(results, maxHits, threshold);
	}

	protected boolean isErrorStatus(final SearchResultList results, final int maxHits)
	{
		final double threshold = 1.0;
		return exceedsThreshold(results, maxHits, threshold);
	}

	private boolean exceedsThreshold(final SearchResultList results, final int maxHits, final double threshold)
	{
		if (maxHits <= 0)
		{
