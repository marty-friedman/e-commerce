...
/**
 * Strategy implementation for function module ERP_LORD_GET_ALL, valid for ERP releases >= ECC605. This product does not
 * support lower releases, lower release support needs to build via extensions!
 */
public class GetAllStrategyERP605 extends GetAllStrategyERP
{
			...
			final ObjectInstances objInstMap, final Map<String, Item> itemMap)
	{
		...
		if (head.isDocumentTypeOrder())
		{
			...
			for (int j = 0; j < numLines; j++)
			{
				...
				final Item item = getParentItem(ttItemVstatComV.getString("HANDLE"), objInstMap, itemMap);

				final String dlvStatus = JCoHelper.getString(ttItemVstatComV, "LFSTA").length() == 0 ? " "
						: JCoHelper.getString(ttItemVstatComV, "LFSTA");
				final String giStatus = JCoHelper.getString(ttItemVstatComV, "WBSTA").length() == 0 ? " "
						: JCoHelper.getString(ttItemVstatComV, "WBSTA");
				final String rjStatus = JCoHelper.getString(ttItemVstatComV, "ABSTA").length() == 0 ? " "
						: JCoHelper.getString(ttItemVstatComV, "ABSTA");
				final String ordInvoiceStatus = JCoHelper.getString(ttItemVstatComV, "FKSAA").length() == 0 ? " "
						: JCoHelper.getString(ttItemVstatComV, "FKSAA");
				final String dlvInvoiceStatus = JCoHelper.getString(ttItemVstatComV, "FKSTA").length() == 0 ? " "
						: JCoHelper.getString(ttItemVstatComV, "FKSTA");
				final String procStatus = JCoHelper.getString(ttItemVstatComV, "GBSTA").length() == 0 ? " "
						: JCoHelper.getString(ttItemVstatComV, "GBSTA");
				...
				shipStatus.init(EStatus.getStatusType(dlvIStatus), EStatus.getStatusType(giIStatus),
				...
				billStatus.init(EStatus.getStatusType(dlvIStatus), EStatus.getStatusType(ordInvIStatus),
						EStatus.getStatusType(dlvInvIStatus), EStatus.getStatusType(rjIStatus));

				item.setBillingStatus(billStatus);

				final OverallStatusOrder overallStatus = getOverallStatusBean();
				overallStatus.init(EStatus.getStatusType(prcStatus), shipStatus, billStatus, EStatus.getStatusType(rjIStatus));
			...
			for (final Map.Entry<String, Item> entry : itemMap.entrySet())
			{
				...
				totalBillingStatus = EStatus.cumulateStatus(cItem.getBillingStatus().getStatus(), totalBillingStatus);
				totalShippingStatus = EStatus.cumulateStatus(cItem.getShippingStatus().getStatus(), totalShippingStatus);
			}
			...
			final String procStatus = JCoHelper.getString(esHvStatComV, "GBSTK").length() == 0 ? " "
					: JCoHelper.getString(esHvStatComV, "GBSTK");
			final String rjHeaderStatus = JCoHelper.getString(esHvStatComV, "ABSTK").length() == 0 ? " "
					: JCoHelper.getString(esHvStatComV, "ABSTK");
	...
	{
		return (ProcessingStatus) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_PROCESSING_STATUS);
	}

	BillingStatus getBillingItemStatus()
	{
		return (BillingStatus) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_BILLING_ITEM_STATUS);
	}

	ShippingStatus getShippingStatusBean()
	{
		return (ShippingStatus) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_SHIPPING_STATUS);
	}

	OverallStatusOrder getOverallStatusBean()
	{
		return (OverallStatusOrder) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_OVERALL_STATUS_ORDER);
	}
