...
public class BasketImpl extends SalesDocumentImpl implements Basket
{
	...
	private static final Log4JWrapper sapLogger = Log4JWrapper.getInstance(BasketImpl.class.getName());
	/**
	...
	{
		return (basketOrderConsistency != null && basketOrderConsistency.isUpdateMissing())
				|| super.isUpdateMissing();
	}
	...
	/**
	 * Callback method for the <code>BusinessObjectManager</code> to tell the object that life is over and that it has to
	...
	public boolean saveAndCommit() throws CommunicationException
	{
		...
		try
		{
			saveWasSuccessful = ((BasketBackend) getBackendService()).saveInBackend(this, true);
			setDirty(true);
	...
	@Override
	public void setBasketOrderConsistency(final BasketOrderConsistency basketOrderConsistency)
	{
	 ...
	 * Checks if basket and order are consistent. If the data was not consistent a update is triggered before the read
	 *
	 * @param force
	...
	private boolean assureConsistency()
	{
		...
		return basketOrderConsistency.assureConsistentData(this);
	}
	...
	{
		throw new CloneNotSupportedException("Clone is not supported for this business object");
	}
