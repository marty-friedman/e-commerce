...
public class DefaultInboundQuoteHelper implements InboundQuoteHelper
{
	...
	@Override
	public Double applyQuoteDiscount(String quoteId, Double discountedPrice, String userUid)
	{
		...
		if (discount > Double.valueOf(0))
		{
			...
			getCommerceQuoteService().applyQuoteDiscount(currentQuote, getUserService().getUserForUID(userUid), discount,
	...
	@Override
	public void addTaxValue(String quoteId)
	{
		final QuoteModel currentQuote = getQuoteService().getCurrentQuoteForCode(quoteId);
		final String taxCode = TAX + currentQuote.getCode();
		
		final TaxValue tax = new TaxValue(taxCode, currentQuote.getTotalTax().doubleValue(), true, currentQuote.getCurrency().toString());
