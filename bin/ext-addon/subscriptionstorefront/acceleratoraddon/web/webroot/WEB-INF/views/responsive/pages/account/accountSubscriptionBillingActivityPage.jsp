...
<spring:url value="/my-account/subscription/{/subscriptionDataId}" var="backToSubscriptionDetails" htmlEscape="false">
...
<div class="account-section">
							...
							<c:forEach items="${billingActivities}" var="billingActivity">
								<tr class="responsive-table-item">
									...
									<td class="responsive-table-cell hidden-xs">
										<c:if test="${billingActivity.paymentStatus eq 'Paid'}">
											<form:form class="download-bill-form" action="${downloadBillingActivityDetailUrl}" method="get">
												<spring:theme var="downloadText" code="text.account.subscription.billingActivity.download" text="Download"/>
												<button type="submit" class="btn btn-primary btn-block" title="${downloadText}">
													${downloadText}
												</button>
												<input type="hidden" name="billingActivityId" value="${ycommerce:encodeHTML(billingActivity.billingId)}"/>
											</form:form>
										</c:if>
									</td>
									<td class="responsive-xs-cell display-none">
										<c:if test="${billingActivity.paymentStatus eq 'Paid'}">
											<form:form class="download-bill-form" action="${downloadBillingActivityDetailUrl}" method="get">
												<spring:theme var="downloadText" code="text.account.subscription.billingActivity.download" text="Download"/>
												<button type="submit" class="btn btn-primary btn-block" title="${downloadText}">
	...
	<div class="account-actions billing-activity-buttons">
		<a href="${backToSubscriptionDetails}" class="btn btn-default">
			<spring:theme code="text.account.subscription.returnToSubscriptionDetails" text="Return To Subscription Details"/>
		</a>
	</div>
</div>