...
<form:form id="idManageAssociatedSubscriptions" class="orderList" method="post" commandName="paymentSubscriptionsForm"
			  action="${changePaymentMethodSubscriptionUrl}">
	<input type="hidden" name="paymentInfoId" value="${ycommerce:encodeHTML(paymentId)}"/>
	<subscription:subcriptionsListingTable subscriptions="${subscriptions}"
