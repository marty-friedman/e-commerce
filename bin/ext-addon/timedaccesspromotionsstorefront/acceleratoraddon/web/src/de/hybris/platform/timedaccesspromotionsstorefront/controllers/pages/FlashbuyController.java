...
public class FlashbuyController extends ProductPageController
{
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		{
			final String redirectURL = XSSFilterUtil
					.filter(request.getRequestURL().replace(request.getRequestURL().lastIndexOf(FLASHBUY_URL),
		...
		final List<String> productKeywords = productModel.getKeywords() == null ? null : productModel.getKeywords().stream()
				.map(k -> k.getKeyword()).collect(Collectors.toList());
		final String metaKeywords = MetaSanitizerUtil.sanitizeKeywords(productKeywords);
	...
	@ResponseBody
	public String enqueue(@PathVariable("productCode") final String encodedProductCode, @RequestBody final String requestBody,
			final HttpServletRequest request, final HttpServletResponse response) throws JSONException
	{
		...
		final String redirectURL = XSSFilterUtil.filter(request.getRequestURL().toString());
		...
		try
		{
			enqueueSuccess = flashbuyPromotionFacade.enqueue(productCode,
					request.getSession().getAttribute(PROMOTION_CODE).toString(), userId, buyQuantity);
			request.getSession().setAttribute(product_promotion_key, "Enequed");
		}
		catch (final CommerceCartModificationException e)
		{
			LOG.error("Couldn't add flash buy product of code to cart.");
			return REDIRECT_PREFIX + redirectURL;
		}
