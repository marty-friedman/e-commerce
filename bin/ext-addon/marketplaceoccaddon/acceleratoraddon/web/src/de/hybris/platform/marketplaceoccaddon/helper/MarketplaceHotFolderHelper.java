...
public class MarketplaceHotFolderHelper
{
	...
	{
		download(response, "text/csv;charset=" + CharEncoding.UTF_8, getOrderExportFile());
	}
	...
	protected void processZIPFile(final HttpServletRequest request)
	{
		...
		try //NOSONAR
		{
			...
			while (entries.hasMoreElements())
			{
				...
				final File entryFile = new File(getOrCreateDir(getMarketplaceHotfolderDir(), getVendorCode()), entry.getName());
	...
	protected void setHeaderDisposition(final HttpServletResponse response, final String filename)
	{
		...
		headerValue += " filename*=" + CharEncoding.UTF_8 + "''" + encodeURIComponent(filename);
