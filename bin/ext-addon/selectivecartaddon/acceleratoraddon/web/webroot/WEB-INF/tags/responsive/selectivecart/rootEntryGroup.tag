            ...
            <c:if test="${not empty entryGroup.orderEntries}">
                <c:forEach items="${entryGroup.orderEntries}" var="entry">
                    <tr>
                        <td>
							<c:choose>
								<c:when test="${entry.cartSourceType == 'WISHLIST'}" >
									<selectivecart:wishlist2Item entry="${entry}" />
								</c:when>
								<c:otherwise>
									<selectivecart:cartItem cartData="${cartData}" entry="${entry}" index="${entryGroup.groupNumber}"/>
								</c:otherwise>
							</c:choose>
                        </td>
                    </tr>
