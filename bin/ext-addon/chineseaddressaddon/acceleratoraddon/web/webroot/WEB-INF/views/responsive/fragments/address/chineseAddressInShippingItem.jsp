...
<span>
<span id="fullNameInShipping"><b>${fn:escapeXml(deliveryAddress.fullnameWithTitle)}</b></span>
	<br>
	<c:choose>
		<c:when test="${fn:toUpperCase(currentLanguage.getIsocode()) eq 'ZH'}">
			...
			<c:if test="${not empty fn:escapeXml(deliveryAddress.city.name)}">
				,&nbsp;${fn:escapeXml(deliveryAddress.city.name)}
			</c:if>
			<c:if test="${empty fn:escapeXml(deliveryAddress.city.name)}">
				,&nbsp;${fn:escapeXml(deliveryAddress.town)}
			</c:if>
			<c:if test="${not empty fn:escapeXml(deliveryAddress.district.name)}">
			...
			<c:if test="${not empty fn:escapeXml(deliveryAddress.cellphone)}">
				,&nbsp;${fn:escapeXml(deliveryAddress.cellphone)}
			</c:if>
			<c:if test="${not empty fn:escapeXml(deliveryAddress.phone)}">
