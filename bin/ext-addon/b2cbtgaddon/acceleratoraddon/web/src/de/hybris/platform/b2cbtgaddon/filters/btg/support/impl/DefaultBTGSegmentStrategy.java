...
public class DefaultBTGSegmentStrategy implements BTGSegmentStrategy
{
	...
	{
		final boolean userHasActiveConsent = btgConsentService.userHasActiveConsent(userService.getCurrentUser());
		// Check if we have already evaluated the BTG segments for this request and if user has Active Consents
		if (!Boolean.TRUE.equals(httpRequest.getAttribute(BTG_SEGMENT_EVALUATED)) && userHasActiveConsent)
		{
	...
	protected BtgConsentService getBtgConsentService()
	{
		return btgConsentService;
	}

	@Required
	public void setBtgConsentService(final BtgConsentService btgConsentService)
	{
		this.btgConsentService = btgConsentService;
	}
}
