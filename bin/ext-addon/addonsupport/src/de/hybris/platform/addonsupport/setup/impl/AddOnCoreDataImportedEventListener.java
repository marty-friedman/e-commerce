...
public class AddOnCoreDataImportedEventListener extends AbstractEventListener<CoreDataImportedEvent>
{
	...
	protected void onEvent(final CoreDataImportedEvent event)
	{
		...
		if (getAddonSystemSetupSupport().getBooleanSystemSetupParameter(event.getContext(), AddOnSystemSetupSupport.IMPORT_SITES))
		{
			...
			{
				writeLogInfo("importing addon [" + parameterData.getConfigExtensionName() + "] configuration for ["
	...
	protected void writeLogInfo(final String msg)
	{
	...
	{
		final CatalogModel productCatalog = getCatalogService()
				.getCatalogForId(importData.getProductCatalogName() + "ProductCatalog");
		...
		final boolean productFilesImported = getAddonConfigDataImportService().executeImport(parameterData.getConfigExtensionName(),
				AddonConfigDataImportType.PRODUCT, parameterData);
		if (productFilesImported && getAddonSystemSetupSupport()
				.getBooleanSystemSetupParameter(context.getSourceEvent().getContext(), AddOnSystemSetupSupport.IMPORT_SYNC_CATALOGS))
		{
		...
		{
			processStoreNames(context, importData, parameterData, productFilesImported, store);
		}

	}


	protected void processStoreNames(final AddOnDataImportEventContext context, final ImportData importData,
			final ImpexMacroParameterData parameterData, final boolean productFilesImported, final String store)
	{
		...
		if (getSupportedChannels().contains(baseSite.getChannel()))
		{
			...
			writeLogInfo("importing addon [" + parameterData.getConfigExtensionName() + "] configuration for [" + store + "]");
			...
			{
				syncCatalogsAndExecuteImport(context, importData, parameterData, baseSite);
			}

			final boolean solrFilesImported = getAddonConfigDataImportService().executeImport(parameterData.getConfigExtensionName(),
					AddonConfigDataImportType.SOLR, parameterData);
			if ((solrFilesImported || productFilesImported) && getAddonSystemSetupSupport().getBooleanSystemSetupParameter(
					context.getSourceEvent().getContext(), AddOnSystemSetupSupport.ACTIVATE_SOLR_CRON_JOBS))
			{
	...
	protected void syncCatalogsAndExecuteImport(final AddOnDataImportEventContext context, final ImportData importData,
			final ImpexMacroParameterData parameterData, final BaseSiteModel baseSite)
	{
		...
		for (final String contentCatalogName : importData.getContentCatalogNames())
		{
			...
			if (cmsSite.getContentCatalogs().contains(contentCatalog))
			{
				...
				final boolean contentFilesImported = getAddonConfigDataImportService()
						.executeImport(parameterData.getConfigExtensionName(), AddonConfigDataImportType.CONTENT, parameterData);
				if (contentFilesImported && getAddonSystemSetupSupport().getBooleanSystemSetupParameter(
						context.getSourceEvent().getContext(), AddOnSystemSetupSupport.IMPORT_SYNC_CATALOGS))
				{
