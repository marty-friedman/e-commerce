...
public class DefaultAddonSampleDataImportService extends AbstractSystemSetup implements AddOnSampleDataImportService
{
	...
	protected void importCommonData(final SystemSetupContext context, final String importRoot)
	{
		...
		if (isExtensionLoaded(loadedExtensionNames, "btgcockpit"))
		{
			importImpexFile(context, importRoot + "/cockpits/btgcockpit/btgcockpit-users.impex", false);
		}
	...
	protected void importProductCatalog(final SystemSetupContext context, final String importRoot, final String catalogName)
	{
		...
		importImpexFile(context, importRoot + PRODUCT_CATALOGS_URL + catalogName
		...
		importImpexFile(context, importRoot + PRODUCT_CATALOGS_URL + catalogName + "ProductCatalog/products-relations.impex", false);
	...
	protected void importStoreInitialData(final SystemSetupContext context, final String importRoot,
			final List<String> storeNames, final String productCatalog, final List<String> contentCatalogs, final boolean solrReindex)
	{
		for (final String storeName : storeNames)
		{
			...
			{
				importImpexFile(context, importRoot + STORES_URL + storeName + "/btg.impex", false);
			}
	...
	protected boolean synchronizeProductCatalog(final SystemSetupContext context, final String catalogName, final boolean sync)
	{
		logInfo(context, "Begin synchronizing Product Catalog [" + catalogName + "] - "
				+ (sync ? SYNCHRONIZING_MSG : INITIALIZING_JOB_MSG));
