		...
		<div class="account-section-content">
			<div class="account-section-form">
				<form:form action="update-email" method="post" commandName="updateEmailForm">
					...
					<input id="recaptchaChallangeAnswered" value="${requestScope.recaptchaChallangeAnswered}" readonly class="input_hidden"/>
					...
					<div class="form-group">
						...
						<select name="emailLanguage" class="form-control">
							<c:forEach items="${languages}" var="lang">
								<option value="${fn:escapeXml(lang.isocode)}" <c:if test="${user.emailLanguage eq lang.isocode}">selected</c:if> >
									<c:out value="${fn:escapeXml(lang.nativeName)}" />
								</option>
							</c:forEach>
						</select>
					</div>					
