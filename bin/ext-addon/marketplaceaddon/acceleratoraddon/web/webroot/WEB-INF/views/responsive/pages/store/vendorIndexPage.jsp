		...
		<div class="vendor-item">
			...
			<c:forEach items="${vendor.logo}" var="media">
				<c:if test="${media.format eq logoFormat}">
					<a href="${vendorUrl}" class="vendors-img-link"><img alt="" src="${media.url}" class="vendors-img" /></a>
				</c:if>
			</c:forEach>
			</c:when>
			<c:otherwise>
				<spring:theme code="img.missingProductImage.responsive.product"
						...
						<a href="${vendorUrl}" class="vendors-img-link"><img alt="" src="${imageUrl}" class="vendors-img" /></a>						
			</c:otherwise>
			</c:choose>
