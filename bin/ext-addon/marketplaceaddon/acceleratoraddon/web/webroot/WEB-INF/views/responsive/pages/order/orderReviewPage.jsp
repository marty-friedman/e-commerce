		...
		<div class="order-review-head">
			<p>
				<c:if test="${consignment.entries.size() eq 1}">
					<spring:theme code="text.order.review.item.headline" arguments="${consignment.entries.size()},${consignment.vendor.name}" argumentSeparator="," htmlEscape="false"/>
				</c:if>
				<c:if test="${consignment.entries.size() gt 1}">
					<spring:theme code="text.order.review.items.headline" arguments="${consignment.entries.size()},${consignment.vendor.name}" argumentSeparator="," htmlEscape="false"/>
				</c:if>			
			</p>
		</div>
			...
			<form:form method="post" id="order-review-form" commandName="orderReviewForm">
				<c:forEach items="${consignment.entries}" var="entry" varStatus="status">
					<div class="order-review-product_list">
						<div class="headline">
							<spring:theme code="text.order.review.item" arguments="${status.count},${entry.orderEntry.product.name}" argumentSeparator=","/>
						</div>
						<div class="row">
							<template:errorSpanField path="productReviewForms[${status.index}].rating">
								...
								<div class="col-md-9">
									<div class="review-rating review-rating-set review-js-ratingCalcSet">
					                    ...
					                    <input type="hidden" class="review-rating-input" name="productReviewForms[${status.index}].rating" value="${orderReviewForm.productReviewForms[status.index].rating}"/>
									</div>
								</div>
						...
						<div class="row">
							<template:errorSpanField path="productReviewForms[${status.index}].comment">
								...
								<div class="col-md-9">
									<textarea name="productReviewForms[${status.index}].comment" class="form-control order-review-textarea" rows="5">${fn:escapeXml(orderReviewForm.productReviewForms[status.index].comment)}</textarea>
								</div>
							</template:errorSpanField>
						</div>
						<input type="hidden" name="productReviewForms[${status.index}].productCode" value="${fn:escapeXml(entry.orderEntry.product.code)}"/>
					</div>
				</c:forEach>
						...
						<template:errorSpanField path="satisfaction">
							...
							<div class="col-md-9">
								<div class="review-rating review-rating-set review-js-ratingCalcSet">
				                    ...
				                    <input type="hidden" class="review-rating-input" name="satisfaction" value="${orderReviewForm.satisfaction}"/>
								</div>
							</div>
						</template:errorSpanField>
					</div>
						...
						<template:errorSpanField path="delivery">
							...
							<div class="col-md-9">
								<div class="review-rating review-rating-set review-js-ratingCalcSet">
				                    ...
				                    <input type="hidden" class="review-rating-input" name="delivery" value="${orderReviewForm.delivery}"/>
								</div>
							</div>
						</template:errorSpanField>
					</div>
						...
						<template:errorSpanField path="communication">
							...
							<div class="col-md-9">
								<div class="review-rating review-rating-set review-js-ratingCalcSet">
				                    ...
				                    <input type="hidden" class="review-rating-input" name="communication" value="${orderReviewForm.communication}"/>
								</div>
							</div>
						</template:errorSpanField>
					</div>
