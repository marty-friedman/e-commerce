...
<%@ taglib prefix="vendor" tagdir="/WEB-INF/tags/addons/marketplaceaddon/responsive/vendor"%>
...
<li class="product__list--item">
	<ycommerce:testId code="test_searchPage_wholeProduct">
		...
		<ycommerce:testId code="searchPage_productName_link_${fn:escapeXml(product.code)}">
		...
		<div class="product__list--price-panel">
			...
			<ycommerce:testId code="searchPage_price_label_${fn:escapeXml(product.code)}">
				<div class="product__listing--price"><product:productListerItemPrice product="${product}"/></div>
			</ycommerce:testId>
		</div>

		<vendor:vendorInfo vendor="${product.vendor}"/>
		
		<c:if test="${not empty product.summary}">
			<div class="product__listing--description">${product.summary}</div>
		</c:if>
