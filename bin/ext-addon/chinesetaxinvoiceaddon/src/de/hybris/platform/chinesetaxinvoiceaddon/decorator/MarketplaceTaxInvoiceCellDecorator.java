 ...
 */
package de.hybris.platform.chinesetaxinvoiceaddon.decorator;
 ...
 *
 */
public class MarketplaceTaxInvoiceCellDecorator implements CSVCellDecorator
{
	private static final Logger LOG = Logger.getLogger(MarketplaceTaxInvoiceCellDecorator.class);

	private final String FLEXIBLE_SEARCH_SERVICE_BEAN = "flexibleSearchService";
	private final String CATALOG_VERSION_SERVICE_BEAN = "catalogVersionService";
	private final String ORDER_DETAIL_BODY_SLOT = "BodyContent-orderdetail";
	...
	public String decorate(final int position, final Map<Integer, String> impexLine)
	{
		...
		try
		{
			catalogVersionModel = getCatalogVersionService().getCatalogVersion("marketplaceContentCatalog", "Staged");
		}
		...
		SearchResult<ContentSlotModel> result = getFlexibleSearchService().search(queryBuilder.toString(), queryParameters);
		...
		{
			LOG.debug("Initialization:" + MarketplaceTaxInvoiceCellDecorator.class.getName() + " found components are "
