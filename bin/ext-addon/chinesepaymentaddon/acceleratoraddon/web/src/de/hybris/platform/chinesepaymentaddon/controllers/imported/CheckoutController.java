 ...
 */
package de.hybris.platform.chinesepaymentaddon.controllers.imported;
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (cookie != null)
		{
			...
			catch (final UnsupportedEncodingException e)
			{
				...
				LOG.error(String.format("Cookie Data could not be decoded : %s", cookie.getValue()));
			}
			catch (final IOException e)
			{
				//we do nothing here and just log the error when catching the exception
				LOG.error("Cookie Data could not be mapped into the Object");
			}
			catch (final Exception e)
			{
				//we do nothing here and just log the error when catching the exception
				LOG.error("Error occurred while creating Anonymous cookie consents");
			}
		}
