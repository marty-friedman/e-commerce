...
public class ReflectionUtilsTest
{
	...
	{
		private final Map<List<String[][]>[][][], Map<Integer[][][][], List<Boolean[]>[]>> complexMapField = new HashMap<List<String[][]>[][][], Map<Integer[][][][], List<Boolean[]>[]>>();
	}
	...
	public void testGetFieldsWithInheritance()
	{
		...
		//then
		Assert.assertNotNull(allFields);
		Assert.assertEquals(7, allFields.size());
		final Set<String> names = allFields.stream().map(Field::getName).collect(Collectors.toSet());
