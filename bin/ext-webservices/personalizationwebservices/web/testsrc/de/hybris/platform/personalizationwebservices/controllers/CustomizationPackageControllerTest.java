...
public class CustomizationPackageControllerTest extends ServicelayerBaseTest
{
	...
	{
		SegmentTriggerDataValidator triggerValidator = new SegmentTriggerDataValidator();
		triggerValidator.setSegmentValidator(new SegmentDataValidator());

		CustomizationPackageValidator customizationValidator = new CustomizationPackageValidator();
		customizationValidator.setCustomizationValidator(new CustomizationDataValidator());
		customizationValidator.setVariationValidator(new VariationDataValidator());
	...
	{
		//given
		CustomizationData customizatinData = new CustomizationData();
		customizatinData.setName(NEW_CUSTOMIZATION_NAME);
		customizatinData.setRank(Integer.valueOf(0));

		//when
		ResponseEntity<CustomizationData> response = controller.createCustomization(CATALOG_ID, CATALOG_VERSION_STAGE_ID,
				customizatinData, uriComponentsBuilder);

		//then
		CustomizationData createdCustomization = response.getBody();
	...
	{
		//given
		final CustomizationData customizationData = creteCustomizationData(NEW_CUSTOMIZATION_ID, NEW_CUSTOMIZATION_NAME, NEW_VARIATION_ID, NEW_VARIATION_NAME, NEW_TRIGGER_ID,
				SEGMENT_ID);
	...
	{
		final CustomizationData customizationData = creteCustomizationData(NEW_CUSTOMIZATION_ID, NEW_CUSTOMIZATION_NAME, NEW_VARIATION_ID, NEW_VARIATION_NAME, null);
	...
	{
		//given
		final CustomizationData customizationData = creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, VARIATION_ID, VARIATION_NAME, TRIGGER_ID, SEGMENT_ID);
	...
	protected void assertLocation(String expected, ResponseEntity<CustomizationData> actual)
	{
