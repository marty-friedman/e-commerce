...
public class DefaultSmarteditI18nFacade implements SmarteditI18nFacade
{
	...
	@Override
	public Map<String, String> getTranslationMap(String languageTag)
	{
	...
	{
		List<SmarteditLanguageData> languageData = ofNullable(getI18nService().getSupportedLocales()).orElse(newHashSet()).stream()
		...
		convertLocalToIsoCodes(languageData);
		return languageData;
	}
	 ...
	 */
	protected void convertLocalToIsoCodes(List<SmarteditLanguageData> languageDataList)
	{
		languageDataList.stream()
				.forEach(languageData ->
				{
					Locale locale = getCommonI18NService().getLocaleForIsoCode(languageData.getIsoCode());
