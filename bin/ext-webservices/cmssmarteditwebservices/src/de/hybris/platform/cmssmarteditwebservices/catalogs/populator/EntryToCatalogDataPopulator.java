			...
			throws ConversionException
	{
		...
		if (!source.getValue().isEmpty())
		{
			final List<CatalogVersionData> versions = source.getValue().stream()
					.map(catalogVersionModel -> {
						final CatalogVersionData catalogVersionData = getCatalogVersionDataConverter().convert(catalogVersionModel);
						catalogVersionData.setUuid(getUniqueItemIdentifierService().getItemData(catalogVersionModel).orElseThrow(
								() -> new UnknownIdentifierException("Cannot generate uuid for component in CatalogAndVersionEntryToCatalogDataPopulator")).getItemId());
						return catalogVersionData;
					}) 
					.sorted(getCatalogVersionDataComparator()).collect(Collectors.toList()); 
