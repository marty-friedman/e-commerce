...
public class ComponentTypeFacadeWithGenericServiceIntegrationTest extends ServicelayerTest
{
	...
	public void shouldFilterAnyAbstractComponentType()
	{
		...
		final List<String> collectedTypes = componentTypes
				.stream()
				.map(ComponentTypeData::getCode)
				.filter(typeCode -> typeService.getComposedTypeForCode(typeCode).getAbstract())
				.collect(Collectors.toList());

		//assert
		assertThat("AbstractComponentTypeFacadeIntegrationTest should filter all abstract classes", collectedTypes, emptyIterable());
	}
}
