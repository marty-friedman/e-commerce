...
@NeedsEmbeddedServer(webExtensions = { CmswebservicesConstants.EXTENSIONNAME, OAuth2Constants.EXTENSIONNAME })
@IntegrationTest
public class CMSItemControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void searchWithNoMaskShouldGetAllCMSComponentsSortByNameASC()
	{
		...
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				...
				.queryParam(URI_SORT, AbstractCMSComponentModel.NAME + ":" + SortDirection.ASC)
				.build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();
	...
	public void searchWithHomeMaskGetsHomePageAndHomePageTemplate()
	{
		...
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				.path(endPoint)
				.queryParam(URI_PAGE_SIZE, DEFAULT_PAGE_SIZE).queryParam(URI_CURRENT_PAGE, DEFAULT_CURRENT_PAGE)
				.queryParam(URI_CATALOG_ID, catalogVersion.getCatalog().getId())
				.queryParam(URI_CATALOG_VERSION, catalogVersion.getVersion())
	...
	public void searchWithParagraphComponentTypeReturnsOnlyTheOneInstance()
	{
		...
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				...
				.queryParam(URI_CATALOG_ID, catalogVersion.getCatalog().getId())
				.queryParam(URI_CATALOG_VERSION, catalogVersion.getVersion())
				.queryParam(URI_TYPECODE, CMSParagraphComponentModel._TYPECODE)
				.build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();
	...
	public void searchWithUnkownTypeReturnsEmptyResults()
	{
		...
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				...
				.queryParam(URI_CATALOG_ID, catalogVersion.getCatalog().getId())
				.queryParam(URI_CATALOG_VERSION, catalogVersion.getVersion())
				.queryParam(URI_TYPECODE, "UnknownComponent")
				.build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();
