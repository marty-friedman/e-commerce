...
import static de.hybris.platform.cmsfacades.constants.CmsfacadesConstants.ROOT_NODE_UID;
...
public class NavigationsControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void testGetRootlNavigationNodes() throws CMSItemNotFoundException
	{
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				.path(replaceUriVariablesWithDefaults(ENDPOINT, new HashMap<>()))
				.queryParam(PARENT_UID_PARAM, ROOT_NODE_UID) //
				.build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();
		...
		assertNode(navigationNodes.get(0), PARENT_UID, ROOT_NODE_UID, 0, true);
	}
	...
	public void testGetOnelNavigationNode() throws CMSItemNotFoundException
	{
		...
		assertNode(navigationNode, PARENT_UID, ROOT_NODE_UID, 0, true);
	...
	public void testUpdateNameForOnelNavigationNode() throws CMSItemNotFoundException
	{
		...
		navigationNode.setParentUid(ROOT_NODE_UID);
		...
		assertNode(data, PARENT_UID, ROOT_NODE_UID, 0, true);
	...
	{
		final String nodeUid = ROOT_NODE_UID;
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				.path(replaceUriVariablesWithDefaults(ENDPOINT, new HashMap<>()))
