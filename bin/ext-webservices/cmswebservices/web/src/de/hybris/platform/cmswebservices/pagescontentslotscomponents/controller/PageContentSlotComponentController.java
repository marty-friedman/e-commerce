...
public class PageContentSlotComponentController
{
	...
	@ApiResponses({
			@ApiResponse(code = 400, message = "If it cannot find the content item (CMSItemNotFoundException) or if there is any validation error (WebserviceValidationException) or if component is not allowed in slot (ComponentNotAllowedInSlotException)."),
	...
	public PageContentSlotComponentData moveComponent(
			...
			@ApiParam(value = "Content slot identifier", required = true) @PathVariable("slotId") final String slotId,
			@ApiParam(value = "Component identifier", required = true) @PathVariable("componentId") final String componentId,
			@ApiParam(value = "PageContentSlotComponentData", required = true) @RequestBody final PageContentSlotComponentData pageContentSlotComponent)
	...
	@ApiResponses({
			@ApiResponse(code = 400, message = "When component item does not exist (CMSItemNotFoundException) or when validation errors are found (WebserviceValidationException)."),
	...
	public @ResponseBody PageContentSlotComponentData addComponentToSlot(
			@ApiParam(value = "PageContentSlotComponentData", required = true) @RequestBody final PageContentSlotComponentData pageContentSlotComponentData,
	...
	@ApiResponses({
			@ApiResponse(code = 400, message = "When the component or slot cannot be found (CMSItemNotFoundException) or when the component slot does not contain the component (ComponentNotFoundInSlotException)")
	})
	public void removeComponent(
			@ApiParam(value = "Content slot identifier containing the component to be removed", required = true) @PathVariable("slotId") final String slotId,
			@ApiParam(value = "Component identifier of the component to be removed", required = true) @PathVariable("componentId") final String componentId)
