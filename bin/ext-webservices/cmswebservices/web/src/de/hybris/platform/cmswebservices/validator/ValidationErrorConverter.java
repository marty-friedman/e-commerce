    ...
    public void populate(Object o, List<ErrorWsDTO> webserviceErrorList) 
    {
        ...
        webserviceErrorList.addAll(
                errors.getValidationErrors().stream().map(validationError -> {
                    ...
                    errorDto = this.createTargetElement();
                    errorDto.setType(TYPE);
                    errorDto.setSubjectType(SUBJECT_TYPE);
                    errorDto.setMessage(this.getMessage(validationError.getErrorCode(), 
                            validationError.getErrorArgs(), currentLocale, validationError.getDefaultMessage()));
