...
public class MapAdapterTest
{
	...
	public void testMarshalParagraph() throws JAXBException {
		...
		final Map<String, String> localizedContent = new HashMap<String, String>();
		localizedContent.put("en", "english content");
		localizedContent.put("fr", "french content");

		componentData.setContent(localizedContent);

		final String json = marshal(componentData);
		...
		final CMSParagraphComponentData componentDataUnmarshaled = (CMSParagraphComponentData)abstractComponentDataUnmarshaled;
		Assert.assertNotNull(componentDataUnmarshaled.getContent());
		Assert.assertTrue(componentDataUnmarshaled.getContent() instanceof Map);

		final Map<String, String> localizedValueMapUnmarshaled = componentDataUnmarshaled.getContent();
		Assert.assertEquals(localizedContent.size(), localizedValueMapUnmarshaled.size());
		Assert.assertNotNull(localizedValueMapUnmarshaled.entrySet().stream().findFirst().get().getValue());
		Assert.assertEquals(localizedContent.get("en"), localizedValueMapUnmarshaled.get("en"));
	}
