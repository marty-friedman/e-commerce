 ...
 */
angular.module('gatewayFactoryModule', ['eventServiceModule', 'functionsModule', 'seConstantsModule'])
    /**
     * @ngdoc object
     ...
     */
    .service('gatewayFactory', function($rootScope, $q, getOrigin, systemEventService, regExpFactory, cloneableUtils, $timeout, $log, $window, $injector, isIframe, WHITE_LISTED_STOREFRONTS_CONFIGURATION_KEY, TIMEOUT_TO_RETRY_PUBLISHING, SMARTEDIT_IFRAME_ID) {
        ...
        var MessageGateway = function MessageGateway(gatewayId) {
             ...
             */
            this.publish = function(eventId, _data, retries, pk) {

                var data = cloneableUtils.makeCloneable(_data);

                if (!angular.equals(data, _data)) {
                    $log.warn("gatewayFactory.publish - Non cloneable payload has been sanitized for gateway " + this.gatewayId + ", event " + eventId + ":", data);
                }
                if (!retries) {
                    retries = 0;
                }
                ...
                try {
                    ...
                    try {
                        ...
                        $log.error(e);
                        $log.error("gatewayFactory.publish - postMessage has failed for gateway " + this.gatewayId + ", event " + eventId + " and data ", data);
                    }

                    promisesToResolve[pk] = deferred;

                    //in case promise does not return because, say, a non ready frame
                    $timeout(function(deferred, retries) {
                    ...
                    }.bind(this, deferred, retries), TIMEOUT_TO_RETRY_PUBLISHING);
                } catch (e) {
                    deferred.reject();
        ...
        return {
            ...
            _isAllowed: function(origin) {
                ...
                return isIframe() || getOrigin() === origin || (whiteListedStorefronts.some(function(allowedURI) {
                    return regExpFactory(allowedURI).test(origin);
                }));
            }
