...
export class DragAndDropCrossOrigin {
	...
	private onTrackMouseInner = (eventId: string, mousePosition: IMousePosition) => {
		 ...
		 */
		this.currentElementHovered = this.yjQuery(this.inViewElementObserver.elementFromPoint(mousePosition));
		const mousePositionInPage: IMousePosition = this.getMousePositionInPage(mousePosition);
	...
	private getMousePositionInPage(mousePosition: IMousePosition): IMousePosition {
		const scrollingElement: JQuery = this.yjQuery(this.$document[0].scrollingElement || this.$document[0].documentElement);
		return {
