...
class JQueryHelper {
	...
	jQuery(selectorTransform?: (selector: string) => jasmine.SpyObj<JQuery<ElementForJQuery>>): JQueryStatic {
	...
	wrap(name: string, ...elementsArray: ElementForJQuery[]): jasmine.SpyObj<JQuery<ElementForJQuery>> {
		...
		const elementsWrapper = jasmine.createSpyObj<JQuery<ElementForJQuery>>(name, this.IDEMPOTENT_METHODS.concat(mockedMethodsOfJQueryWrapperNames));

		filteredElementsArray.forEach((element, index) => {
			elementsWrapper[index] = element;
		});

		(elementsWrapper as any).length = filteredElementsArray.length;

		this.DEFAULT_MOCKED_JQUERY_FUNCTIONS.forEach((methodName: keyof JQuery) => {
			elementsWrapper[methodName].and.returnValue({});
		});

		if (elementWithMocks) {
			lodash.forEach(elementWithMocks.mockedMethodsOfJQueryWrapper, (value: Primitive, methodName: keyof JQuery) => {
				elementsWrapper[methodName].and.returnValue(value);
			});
		}

		this.IDEMPOTENT_METHODS.forEach((methodName: keyof JQuery) => {
