...
import {PerspectiveService, StorageService} from 'smarteditcontainer/services';
import {IPerspective} from 'smarteditcommons';
...
describe('outer perspectiveService', () => {
	...
	const systemEventService: any = jasmine.createSpyObj('systemEventService', ['sendAsynchEvent', 'registerEventHandler']);
	const gatewayProxy: any = jasmine.createSpyObj('gatewayProxy', ['initForService']);
	const iFrameManager: any = jasmine.createSpyObj('iFrameManager', ['showWaitModal', 'hideWaitModal']);
	const featureService: any = jasmine.createSpyObj('featureService', ['enable', 'disable', 'getFeatureProperty']);
	...
	beforeEach(() => {
		perspectiveService = new PerspectiveService($log, $q, isBlank, uniqueArray, systemEventService,
			featureService, gatewayProxy, iFrameManager, storageService, crossFrameEventService, NONE_PERSPECTIVE,
	...
	describe('initialization', () => {
		...
		it('initializes and invokes gatewayProxy', () => {
			...
			expect(systemEventService.registerEventHandler.calls.count()).toBe(2);
		});
		...
		it('_registerEventHandlers will register event handlers', () => {
			...
			spyOn(perspectiveService as any, '_onLogoutPerspectiveCleanup').and.callFake(function() {/**/});
			...
			const clearCallback = systemEventService.registerEventHandler.calls.argsFor(0)[1];
			clearCallback();

			const logoutCallback = systemEventService.registerEventHandler.calls.argsFor(1)[1];
			logoutCallback();
			expect((perspectiveService as any)._onLogoutPerspectiveCleanup).toHaveBeenCalled();
		});
	...
	describe('switchTo', () => {
		beforeEach(() => {
			...
			iFrameManager.showWaitModal.calls.reset();
			iFrameManager.hideWaitModal.calls.reset();
			featureService.enable.calls.reset();
			featureService.disable.calls.reset();
		});
	...
	describe('clearActivePerspective', () => {
		it('WILL delete active perspective', () => {
			// GIVEN
			const perspective0 = {
				key: 'persp0',
				nameI18nKey: 'persp0',
	...
	describe('cleanup', () => {
		it('WHEN _onLogoutPerspectiveCleanup is called THEN it disables all active features and removes current active perspective', (done) => {
			// GIVEN
			...
			spyOn((perspectiveService as any), '_fetchAllFeatures').and.callFake((perspectiveName: string, tempArray: string[]) => {
			...
			// WHEN
			(perspectiveService as any)._onLogoutPerspectiveCleanup().then(() => {
				expect(featureService.disable.calls.count()).toBe(2);
