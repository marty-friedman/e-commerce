...
import 'jasmine';
import {IDragAndDropEvents, IMousePosition} from 'smarteditcommons';
import {DragAndDropCrossOrigin} from 'smarteditcontainer/services';
import {domHelper, jQueryHelper, ElementForJQuery} from 'testhelpers';

describe('DragAndDropCrossOriginOuter', () => {

	let iFrameManager: jasmine.SpyObj<any>;
	...
	const lodash = (window as any)._ || (window as any).smarteditLodash;
	...
	beforeEach(() => {
		...
		iFrameManager = jasmine.createSpyObj('iFrameManager', ['getIframe', 'isCrossOrigin']);
		iFrameManager.getIframe.and.returnValue(iframeWrapper);
	...
	describe('callbacks', () => {
		...
		it('onDragEnd in cross origin, dragover and drop listeners are removed from the drag area', () => {
			...
			iFrameManager.isCrossOrigin.and.returnValue(true);
		...
		it('onDragStart will stop if not cross origin', () => {
			...
			iFrameManager.isCrossOrigin.and.returnValue(false);
