...
import {IExtensibleResourceClass, Page, Pageable, Payload} from 'smarteditcommons';
...
describe('test RestServiceFactory ', () => {
	...
	beforeEach(() => {
		...
		lodash = (window as any)._ || (window as any).smarteditLodash;
	...
	it('$resource is initialized with a custom page method', () => {
		...
		let transformedResponse = getMethodMap().page.transformResponse(undefined, () => {
			return {
		...
		transformedResponse = getMethodMap().page.transformResponse(JSON.stringify(data), () => {
			return {
				header1: 'header1Value'
			};
		});
