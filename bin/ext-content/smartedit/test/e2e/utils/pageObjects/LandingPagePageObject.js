...
module.exports = (function() {
    ...
    var leftToolbar = require('../components/LeftToolbar.js');

    var componentObject = {};

    componentObject.constants = {
        LANDING_PAGE_PATH: 'test/e2e/landingPage/landingPageTest.html',
    ...
    componentObject.elements = {
        ...
        getSiteSelectorOptionByName: function(siteName) {
            return this.getSiteSelector().element(by.cssContainingText('.select2-result-single span', siteName));
        },
        ...
        getCatalogContainerByName: function(catalogName) {
            return this.getCatalogByName(catalogName).element(by.css('.yCollapsibleContainer__group'));
        },
        getCatalogTitle: function(catalogName) {
            return this.getCatalogByName(catalogName).element(by.css('.yCollapsibleContainer__header'));
        },
        getCatalogThumbnail: function(catalogName) {
            return this.getCatalogByName(catalogName).element(by.css('catalog-versions-thumbnail-carousel .se-active-catalog-version-container__thumbnail'));
        },
        ...
        getCatalogVersionTemplateByName: function(catalogName, catalogVersion, item) {
            return this.getCatalogVersion(catalogName, catalogVersion)
                .element(by.cssContainingText('div', item));
        },
    ...
    componentObject.actions = {
        ...
        openAndBeReady: function() {
            browser.get(componentObject.constants.LANDING_PAGE_PATH);
        ...
        selectSite: function(siteName) {
            return this.openSiteSelector().then(function() {
                return browser.click(componentObject.elements.getSiteSelectorOptionByName(siteName));
            });
        },
        ...
        navigateToStorefrontViaThumbnail: function(catalogName) {
            return browser.click(componentObject.elements.getCatalogThumbnail(catalogName)).then(function() {
                browser.waitForWholeAppToBeReady();
                browser.waitForUrlToMatch(/\/storefront/);
            });
        },
        navigateToStorefrontViaHomePageLink: function(catalogName, catalogVersion) {
            return browser.click(componentObject.elements.getHomePageLink(catalogName, catalogVersion)).then(function() {
                browser.waitForWholeAppToBeReady();
                browser.waitForUrlToMatch(/\/storefront/);
            });
        },
        ...
        navigateToLandingPage: function() {
            return leftToolbar.actions.openLeftToolbar().then(function() {
                return leftToolbar.actions.clickSitesButton().then(function() {
                    browser.waitForContainerToBeReady();
                    browser.waitForUrlToMatch(/^(?!.*storefront)/);
                }.bind(this));
            }.bind(this));
    ...
    componentObject.assertions = {
        ...
        expectedSiteIsSelected: function(siteName) {
            expect(componentObject.elements.getSiteSelectorValue()).toBe(siteName, "Selected site doesn't match expected one.");
        },
        selectedSiteHasRightNumberOfCatalogs: function(expectedNumberOfCatalogs) {
            browser.waitUntil(function() {
                var actualNumberOfCatalogs = null;
                return componentObject.elements.getNumberOfCatalogsDisplayed().then(function(_actualNumberOfCatalogs) {
        ...
        catalogIsExpanded: function(catalogName) {
            expect(componentObject.utils.isCatalogExpanded(catalogName)).toBe(true, "Expected catalog to be expanded");
        },
        catalogIsNotExpanded: function(catalogName) {
            expect(componentObject.utils.isCatalogExpanded(catalogName)).toBe(false, "Expected catalog to be expanded");
        },
        catalogVersionContainsItem: function(catalogName, catalogVersion, item) {
            expect(componentObject.elements.getCatalogVersionTemplateByName(catalogName, catalogVersion, item).isPresent()).toBe(true, "Expected template to be displayed in catalog version.");
        },
        ...
        assertRightUrlIsLoaded: function(expectedSite, expectedCatalog, expectedCatalogVersion) {
            var expectedUrl = '/storefront/' + expectedSite + '/' + expectedCatalog + '/' + expectedCatalogVersion;
            expect(componentObject.elements.getBrowserUrl()).toContain(expectedUrl);
        }
    };

    componentObject.utils = {
        isCatalogExpanded: function(catalogName) {
            return this.hasClass(componentObject.elements.getCatalogContainerByName(catalogName), 'panel-open');
        },
