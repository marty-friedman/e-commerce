...
logoutUser = function() {
    return browser.switchToParent().then(function() {
        return browser.click(by.css('a.se-sign-out__link'));
    });
};

redirectUserToLandingPage = function() {
    return browser.switchToParent().then(function() {
        return browser.click(by.id('nav-expander')).then(function() {
            return browser.click(by.css('.se-left-nav a[data-ng-click="showSites()"]'));
        });
    });
};
