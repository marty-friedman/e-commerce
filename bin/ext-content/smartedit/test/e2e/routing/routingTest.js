...
describe('Test Routing', function() {
    ...
    xit('navigates to custom view from the inner smartedit iframe', function() {
        expect(element(by.css('.ySmartEditTitleToolbar')).isPresent()).toBe(true);
        browser.switchToIFrame().then(function() {
            browser.click(by.id('navigateButtonInner')).then(function() {
                expect(browser.getCurrentUrl()).toContain('/customView');
                browser.waitForAbsence(element(by.css('.ySmartEditTitleToolbar')));
                expect(element(by.css('.content')).getText()).toBe('custom view 2');
            });
        });
    }).pend('Fails due to chromedriver 2.19, upgrade to 2.21 then re-enable this test');
    ...
    xit('navigates to new view from the custom non smartedit iframe', function() {
        browser.setLocation('/test').then(function() {
