...
angular.module('backendMocks', ['ngMockE2E', 'functionsModule', 'resourceLocationsModule', 'languageServiceModule'])
    .run(function($httpBackend, filterFilter, parseQuery, I18N_RESOURCE_URI, languageService) {
        ...
        $httpBackend.whenPOST(/previewApi/).respond(function(method, url, data) {
            ...
            component.uuid = Math.random().toString(36).substring(7);
            return [200, component];
        });

        $httpBackend.whenGET(/previewApi\/([\w]+)/).respond(function(method, url) {
        ...
        $httpBackend.whenPUT(/previewApi\/([\w]+)/).respond(function(method, url, data) {
            var id = /previewApi\/([\w]+)/.exec(url)[1];
            component = JSON.parse(data);
            if (id === component.uuid) {
                return [200, {}];
