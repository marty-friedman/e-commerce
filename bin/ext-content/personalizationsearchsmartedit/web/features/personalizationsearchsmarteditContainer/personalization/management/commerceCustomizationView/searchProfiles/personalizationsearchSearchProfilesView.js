angular
    ...
    .controller('personalizationsearchSearchProfilesController', function($q, $scope, $filter, personalizationsearchRestService, personalizationsmarteditMessageHandler, waitDialogService, personalizationsearchSearchProfilesContextService, personalizationsmarteditContextService, PaginationHelper, SEARCH_PROFILE_ACTION_TYPE, PERSONALIZATION_COMMERCE_CUSTOMIZATION_ACTION_STATUSES) {
        ...
        $scope.initUiSelect = function(uiSelectController) {
            ...
            $scope.availableSearchProfiles = JSON.parse(JSON.stringify($scope.availableSearchProfiles));
        };
        ...
        $scope.treeOptions = {
            dragStart: function() {
                $scope.destEl = undefined;
            },
            beforeDrop: function(e) {
                $scope.destEl = $scope.searchProfileActions[e.dest.index];
            },
            dropped: function(e) {
                if (e.source.index !== e.dest.index) {
                    //update backing actions array
                    var sourceEl = e.source.nodeScope.$modelValue;
                    $scope.actions.splice($scope.destEl.baseIndex, 0, $scope.actions.splice(sourceEl.baseIndex, 1)[0]);
