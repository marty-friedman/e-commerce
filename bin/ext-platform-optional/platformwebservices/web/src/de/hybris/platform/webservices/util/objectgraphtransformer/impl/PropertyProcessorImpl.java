...
public class PropertyProcessorImpl implements PropertyProcessor
{
	...
	private void writeValueToTarget(final PropertyContextImpl pCtx, final Object target, Object value)
	{
		...
		// in order to allow model layer to ignore duplicate writes we need to invoke READ on all attributes first
		try
		{
			Method readMethod = pMap.getTargetConfig().getReadMethod();
			if( readMethod !=  null )
			{
				readMethod.invoke(target, null);
			}
		}
		catch( Exception e)
		{
			log.warn("Error reading attribute " + pCtx.createTargetPathString()+" before writing",e);
		}
