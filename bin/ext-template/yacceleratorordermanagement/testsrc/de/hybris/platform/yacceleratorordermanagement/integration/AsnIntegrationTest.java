...
public class AsnIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
	...
	@Test
	public void testCancelAsn_OrderSuspended_SingleEntry() throws InterruptedException
	{
		//Given
		createDefaultAsn();
		order = sourcingUtil.createCameraShippedOrder();
		final OrderProcessModel orderProcessModel = sourcingUtil.runOrderProcessForOrderBasedPriority(order, OrderStatus.READY);
		//When
		final AdvancedShippingNoticeModel cancelledAsn = asnService.cancelAsn(Asns.INTERNAL_ID_CAMERA_BOSTON);
		//Then
	...
	@Test
	public void testCancelAsn_OrderSuspended_OneConsignmentMultiEntries() throws InterruptedException
	{
		//Given
		advancedShippingNotice = asns.CameraAndMemoryCardAsn_Boston();
		asnService.processAsn(advancedShippingNotice);
		order = sourcingUtil.createCameraAndMemoryCardShippingOrder();
		final OrderProcessModel orderProcessModel = sourcingUtil.runOrderProcessForOrderBasedPriority(order, OrderStatus.READY);
		//When
		final AdvancedShippingNoticeModel cancelledAsn = asnService.cancelAsn(Asns.INTERNAL_ID_CAMERA_MEMORY_CARD_BOSTON);
		//Then Verify Order
		assertEquals(AsnStatus.CANCELLED, cancelledAsn.getStatus());
		sourcingUtil.waitForOrderStatus(orderProcessModel, order, OrderStatus.SUSPENDED, timeOut);
		modelService.refresh(order);
		...
		//Then Verify Consignment
		assertTrue(verifyOrderAndConsignment
				.verifyConsignment_Camera_MemoryCard(order, CODE_BOSTON, CAMERA_QTY, Long.valueOf(0L), Long.valueOf(0L),
						MEMORY_CARD_QTY, Long.valueOf(0L), Long.valueOf(0L)).booleanValue());
	}

	@Test
	@Ignore  //TODO OMSE-2833
	public void testCancelMultiAsn() throws InterruptedException
	{
		//Given create 2 asn, and sourced from them
		advancedShippingNotice = asns.CameraAsn_Montreal();
		advancedShippingNotice2 = asns.CameraAsn_Boston();
		asnService.processAsn(advancedShippingNotice);
		asnService.processAsn(advancedShippingNotice2);
		order = sourcingUtil.createOrder(orders.Camera_Shipped(6L));
		final OrderProcessModel orderProcessModel = sourcingUtil.runOrderProcessForOrderBasedPriority(order, OrderStatus.READY);
		//When cancel first asn
		final AdvancedShippingNoticeModel cancelledAsn = asnService.cancelAsn(Asns.INTERNAL_ID_CAMERA_BOSTON);
		//Then Verify Order
		assertEquals(AsnStatus.CANCELLED, cancelledAsn.getStatus());
		sourcingUtil.waitForOrderStatus(orderProcessModel, order, OrderStatus.SUSPENDED, timeOut);
		...
		assertTrue(
				verifyOrderAndConsignment.verifyConsignment_Camera(order, CODE_MONTREAL, Long.valueOf(0L), CAMERA_QTY, CAMERA_QTY));
		//When cancel second asn
		final AdvancedShippingNoticeModel cancelledAsn2 = asnService.cancelAsn(Asns.INTERNAL_ID_CAMERA_MONTREAL);
		//Then Verify Order
		assertEquals(AsnStatus.CANCELLED, cancelledAsn2.getStatus());
		sourcingUtil.waitForOrderStatus(orderProcessModel, order, OrderStatus.SUSPENDED, timeOut);
		modelService.refresh(order);
		order.getConsignments().forEach(this::refreshConsignmentAndEntries);
		assertEquals(OrderStatus.SUSPENDED, order.getStatus());
		...
		assertTrue(verifyOrderAndConsignment
				.verifyConsignment_Camera(order, CODE_BOSTON, CAMERA_QTY, Long.valueOf(0L), Long.valueOf(0L)));
		assertTrue(
				verifyOrderAndConsignment.verifyConsignment_Camera(order, CODE_MONTREAL, CAMERA_QTY, Long.valueOf(0L), Long.valueOf(0L)));
