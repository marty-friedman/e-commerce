...
class ProductTests extends BaseWSTest {
	...
	void testSearchProductsBasicXML() {
		def con = testUtil.getSecureConnection("/products", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsSpellingJSON() {
		def con = testUtil.getSecureConnection("/products?query=somy", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsSpellingXML() {
		def con = testUtil.getSecureConnection("/products?query=somy", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	@Test
	void testSearchProductsAutoSuggestRequiredParameterMissing() {
		def con = testUtil.getSecureConnection("/products/suggest?noneterm=ta", 'GET', 'JSON', HttpURLConnection.HTTP_BAD_REQUEST)
		def error = con.errorStream.text;

		assert error.contains('Required String parameter')
	}

	@Test
	void testSearchProductsAutoSuggestXML() {
		def con = testUtil.getSecureConnection("/products/suggest?term=ta", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsAutoSuggestWithLimitXML() {
		def con = testUtil.getSecureConnection("/products/suggest?term=ta&max=1", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsAutoSuggestJSON() {
		def con = testUtil.getSecureConnection("/products/suggest?term=ta", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsAutoSuggestWithLmitJSON() {
		...
		def con = testUtil.getSecureConnection("/products/suggest?term=ta&max=1", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsPaginationXML() {
		...
		(0..(response.pagination.totalPages.toInteger())).each { pageNumber ->
			...
			con = testUtil.getSecureConnection("/products?currentPage=${pageNumber}", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsSortXML() {
		def con = testUtil.getSecureConnection("/products?query=camera&sort=topRated", 'GET', 'XML', HttpURLConnection.HTTP_OK)
		...
		response.sorts.sort.each { sort ->
			con = testUtil.getSecureConnection("/products?query=camera:${sort.code}", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	void testSearchProductsSortJSON() {
		...
		response.sorts.each { sort ->
			con = testUtil.getSecureConnection("/products?query=camera:${sort.code}", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
			response = testUtil.verifiedJSONSlurper(con, false, false)
	...
	void testProductDetailsXML() {
		def con = testUtil.getSecureConnection("/products?query=camera", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	void testProductDetailsJSON() {
		def con = testUtil.getSecureConnection("/products?query=camera", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
	...
	void testProductDetailsSpecificXML() {
		def con = testUtil.getSecureConnection("/products/872912a", 'GET', 'XML', HttpURLConnection.HTTP_OK) //some SD CARD
		def response = testUtil.verifiedXMLSlurper(con, false, false)
	...
	void testProductDetailsSpecificSJON() {
		def con = testUtil.getSecureConnection("/products/872912a", 'GET', 'JSON', HttpURLConnection.HTTP_OK) //some SD CARD
		def response = testUtil.verifiedJSONSlurper(con, false, false)
	...
	void testProductExtendedDetailsSpecificSJON() {
		def con = testUtil.getSecureConnection("/products/872912a?options=BASIC,DESCRIPTION,GALLERY,CATEGORIES,PROMOTIONS,STOCK,REVIEW,CLASSIFICATION,REFERENCES,PRICE", 'GET', 'JSON', HttpURLConnection.HTTP_OK) //some SD CARD
		def response = testUtil.verifiedJSONSlurper(con, false, false)
		...
		def classification = response.classifications.find{it.code == '834'}
	...
	void testGetProductByCodeWithReferencesXML() {
		def trustedClientCredentialsToken = testUtil.getTrustedClientCredentialsToken();
		def code = '2053226'; // electronicsstore extension, products-relations.impex, INSERT_UPDATE ProductReference
		def con = testUtil.getSecureConnection("/products/${code}?options=REFERENCES", "GET", "XML", HttpURLConnection.HTTP_OK, null, null, trustedClientCredentialsToken)
		def response = testUtil.verifiedXMLSlurper(con)
		assert response != null
	...
	void testCreateReview() {
		...
		def code = '816780';
		def con = testUtil.getSecureConnection("/products/${code}/reviews", "POST", "JSON", HttpURLConnection.HTTP_OK, postBody);
		def response = testUtil.verifiedJSONSlurper(con)
		assert response != null
	...
	void testCreateReviewXML() {
		def postBody = "alias=krzys&rating=4&comment=perfect&headline=samplereview"
		def code = '816780';
		def con = testUtil.getSecureConnection("/products/${code}/reviews", "POST", "XML", HttpURLConnection.HTTP_OK, postBody);
		def response = testUtil.verifiedXMLSlurper(con)
		assert response != null
		assert response.alias == "krzys"
		assert response.rating == "4.0"
	...
	void testSearchProductStockByLocation() {
		...
		def code = '872912a';
		def con = testUtil.getSecureConnection("/products/${code}/nearLocation${urlParams}", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedJSONSlurper(con, false, false)
	...
	void testSearchProductStockByLocationXML() {
		def urlParams = '?location=wsTestLocation&pageSize=5&currentPage=0';
		def code = '872912a';
		def con = testUtil.getSecureConnection("/products/${code}/nearLocation${urlParams}", 'GET', 'XML', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedXMLSlurper(con, false, false)
		assert response.product.code == code;
		assert response.product.name == 'Secure Digital Card 2GB';
	...
	void testSearchProductStockByLocationGeoCode() {
		...
		def code = '872912a';
		def con = testUtil.getSecureConnection("/products/${code}/nearLatLong${urlParams}", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedJSONSlurper(con, false, false)
		assert response.product.code == code;
		assert response.product.name == 'Secure Digital Card 2GB';
		assert response.stores.size() > 0
	...
	void testSearchProductStockByLocationGeoCodeXML() {
		def urlParams = '?latitude=35.6816951&longitude=139.7650482&pageSize=5&currentPage=0';
		def code = '872912a';
		def con = testUtil.getSecureConnection("/products/${code}/nearLatLong${urlParams}", 'GET', 'XML', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedXMLSlurper(con)
		assert response.product.code == code;
		assert response.product.name == 'Secure Digital Card 2GB';
		assert response.stores.size() > 0
	...
	void testGetStockLevelForStore() {
		...
		def code = 2006139
		def con = testUtil.getSecureConnection("/products/${code}/stock?storeName=${STORE_NAME}", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
		def response = testUtil.verifiedJSONSlurper(con, false, false)
	...
	void testFailGetStockLevelWhenWrongStoreName() {
		def access_token = testUtil.getClientCredentialsToken()
		def code = 2006139
		def con = testUtil.getSecureConnection("/products/${code}/stock?storeName=WrongStoreName", 'GET', 'XML', HttpURLConnection.HTTP_BAD_REQUEST)
		def error =con.errorStream.text;
		def response = new XmlSlurper().parseText(error)
