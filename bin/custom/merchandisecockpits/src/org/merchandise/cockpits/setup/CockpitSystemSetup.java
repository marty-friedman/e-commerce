 ...
 */
package org.merchandise.cockpits.setup;
...
import org.merchandise.cockpits.constants.MerchandiseCockpitsConstants;
 ...
 */
@SystemSetup(extension = MerchandiseCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MerchandiseCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MerchandiseCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MerchandiseCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/merchandisecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
