...
import {IContextualMenuButton, IContextualMenuConfiguration, IFeatureService} from 'smarteditcommons';
import {ComponentHandlerService, ContextualMenuService} from 'smartedit';
import {AssetsService} from 'cmscommons';
import {cmsSmarteditServicesModule} from 'cmssmartedit/services/cmsSmarteditServicesModule';
import 'cmssmartedit/services/deprecatedSince-6.7';
		...
		featureService.addContextualMenuButton({
			...
			condition: (configuration: IContextualMenuConfiguration) => {
				...
				if (smarteditCatalogVersionUuid) {
					return componentHandlerService.getCatalogVersionUUIDFromPage().then(function(uuid: string) {
