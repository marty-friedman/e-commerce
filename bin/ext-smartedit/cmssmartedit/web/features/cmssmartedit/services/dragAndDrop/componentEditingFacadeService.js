...
angular.module('componentEditingFacadeModule', [
    ...
    .service('componentEditingFacade', function($log, $q, $translate, copy, ComponentService, componentHandlerService, restServiceFactory, editorModalService, renderService, alertService, PAGES_CONTENT_SLOT_COMPONENT_RESOURCE_URI, componentVisibilityAlertService, slotVisibilityService, sharedDataService) {
        ...
        function renderSlots(slots, sourceComponentId, targetSlotId, showSuccess) {
            return renderService.renderSlots(slots).then(function() {
                return slotVisibilityService.reloadSlotsInfo().then(function() {
                    if (showSuccess) {
                        alertService.showSuccess(_generateSuccessMessage(sourceComponentId, targetSlotId));
                    }
                    return $q.when();
                }, function(e) {
                    $log.error('componentEditingFacadeModule.renderSlots::slotVisibilityService.reloadSlotsInfo');
                    $log.error(e);
                    return $q.reject(e);
                });
            }, function(e) {
                $log.error('componentEditingFacadeModule.renderSlots::renderService.renderSlots - targetSlotId:', targetSlotId);
                $log.error(e);
                return $q.reject(e);
            });
        }
        ...
        this.addNewComponentToSlot = function(slotInfo, catalogVersionUuid, componentType, position) {
            ...
            return editorModalService.open(componentAttributes, slotInfo.targetSlotUUId, position).then(function(response) {
                return renderSlots([slotInfo.targetSlotId], response.uid, slotInfo.targetSlotId, true);
            }).catch(function(response) {
                alertService.showDanger(_generateErrorMessage(0, slotInfo.targetSlotId, response));
                return $q.reject();
            });
        };
                ...
                return ComponentService.addExistingComponent(pageId, dragInfo.componentId, targetSlotId, position).then(
                    function() {
                        return ComponentService.loadComponentItem(dragInfo.componentUuid).then(function(item) {
                            ...
                            return renderSlots(targetSlotId, dragInfo.componentId, targetSlotId, true);
                        }, function(response) {
                            alertService.showDanger(_generateErrorMessage(dragInfo.componentId, targetSlotId, response));
                            return $q.reject();
                        });
                    },
                    function(response) {
            ...
            return ComponentService.loadComponentItem(componentProperties.dragInfo.componentUuid).then(function(_component) {
                return sharedDataService.get('experience').then(function(experience) {
                    ...
                    return editorModalService.open(componentAttributes, componentProperties.targetSlotId, componentProperties.position).then(function(item) {
                        ...
                        return renderSlots(componentProperties.targetSlotId, item.uid, componentProperties.targetSlotId, true);
                    }).catch(function(response) {
                        alertService.showDanger(_generateErrorMessage(component.uid, componentProperties.targetSlotId, response));
                        return $q.reject();
                    });
                });
            }, function(response) {
                alertService.showDanger(_generateErrorMessage(componentProperties.dragInfo.componentId, componentProperties.targetSlotId, response));
                return $q.reject();
            });
        };
        ...
        this.moveComponent = function(sourceSlotId, targetSlotId, componentId, position) {
            ...
            return componentHandlerService.getPageUID().then(function(pageId) {
                return _contentSlotComponentsRestService.update({
                    ...
                    function() {
                        return renderSlots([sourceSlotId, targetSlotId], componentId, targetSlotId);
                    },
                    function(response) {
