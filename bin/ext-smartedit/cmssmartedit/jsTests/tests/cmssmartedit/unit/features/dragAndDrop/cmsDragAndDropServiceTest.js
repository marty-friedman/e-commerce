...
describe('cmsDragAndDropService', function() {
    ...
    beforeEach(function() {
        var harness = AngularUnitTestHelper
            ...
            .mock('systemEventService', 'unRegisterEventHandler')
            .mock('systemEventService', 'sendAsynchEvent').and.returnResolvedPromise(true)
            .mock('systemEventService', 'sendSynchEvent').and.returnResolvedPromise(true)
    ...
    it('WHEN unregister is called THEN the service is cleaned.', function() {
        // Arrange
        var expectedResult = 'some result';
        spyOn(cmsDragAndDropService, '_onOverlayUpdate').and.returnValue(expectedResult);
        ...
        expect(mocks.systemEventService.unRegisterEventHandler).toHaveBeenCalledWith('overlayRerendered', jasmine.any(Function));
        expect(mocks.systemEventService.unRegisterEventHandler).toHaveBeenCalledWith('COMPONENT_REMOVED_EVENT', jasmine.any(Function));

        var overlayRenderedCallbackResult = mocks.systemEventService.unRegisterEventHandler.calls.argsFor(0)[1]();
        var componentRemovedCallbackResult = mocks.systemEventService.unRegisterEventHandler.calls.argsFor(1)[1]();
        expect(overlayRenderedCallbackResult).toBe(expectedResult);
        expect(componentRemovedCallbackResult).toBe(expectedResult);
    });
    ...
    describe('apply', function() {
        ...
        it('WHEN apply is called THEN the page is prepared for the drag and drop operations', function() {
            ...
            expect(mocks.systemEventService.registerEventHandler).toHaveBeenCalledWith('overlayRerendered', jasmine.any(Function));
            expect(mocks.systemEventService.registerEventHandler).toHaveBeenCalledWith('COMPONENT_REMOVED_EVENT', jasmine.any(Function));
    ...
    describe('event handlers', function() {
        ...
        it('WHEN mouse leaves drag area THEN the slot is cleared', function() {
            ...
            expect(mocks.systemEventService.sendSynchEvent.calls.count()).toBe(2);
            expect(mocks.systemEventService.sendSynchEvent.calls.argsFor(0)[0]).toEqual('HIDE_SLOT_MENU');
            expect(mocks.systemEventService.sendSynchEvent.calls.argsFor(1)[0]).toEqual('CMS_DRAG_LEAVE');
        });
        ...
        it('WHEN onStop is called THEN it cleans the drag operation', function() {
            ...
            // Assert
            expect(mocks.systemEventService.sendSynchEvent).toHaveBeenCalledWith('contractChangeListenerRestartProcess');
    ...
    it('WHEN _initializeDragOperation is called THEN the page is prepared for dragging components', function() {
        ...
        expect(mocks.systemEventService.sendAsynchEvent).toHaveBeenCalledWith('CMS_DRAG_STARTED');
    });
    ...
    it('WHEN _cleanDragOperation is called THEN the page is cleaned up', function() {
        ...
        expect(mocks.systemEventService.sendAsynchEvent).toHaveBeenCalledWith('CMS_DRAG_STOPPED');
