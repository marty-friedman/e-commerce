	...
	public ActionResult<String> perform(final ActionContext<Object> ctx)
	{
		try
		{
			...
			if (YFormDefinitionStatusEnum.DISABLED.equals(yformDefinition.getStatus()))
			{
				...
				NotificationUtils.notifyUser(NotificationUtils.getWidgetNotificationSource(ctx), ARCHIVE_YFORM_DEFINITION_ENABLE, NotificationEvent.Level.SUCCESS, yformDefinition);
			}
			else
			{
				yformService.setFormDefinitionStatus(applicationId, formId, YFormDefinitionStatusEnum.DISABLED);

				NotificationUtils.notifyUser(NotificationUtils.getWidgetNotificationSource(ctx), ARCHIVE_YFORM_DEFINITION_DISABLE, NotificationEvent.Level.SUCCESS, yformDefinition);
			}
		...
		catch (final Exception e)
		{
			...
			NotificationUtils.notifyUser(NotificationUtils.getWidgetNotificationSource(ctx), ARCHIVE_YFORM_DEFINITION_ERROR, NotificationEvent.Level.FAILURE, e);
